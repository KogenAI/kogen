...
defmodule CodegenTestHarness.HarnessParity.HugoMinimalTest do
  @moduledoc """
  Asserts the hugo-minimal build (static stack, Hugo site) produces the same
  observable contract under both the claude and pi harnesses (exit 0, at least
  one content Markdown file present, >= 1 commit beyond the scaffold's initial
  commit). A Pi-only launcher/dispatch break reds here instead of shipping.
  Legitimate per-harness divergence is suppressed only via a `known_divergent.exs`
  tuple with a written reason.
  """
  use ExUnit.Case, async: false
  ...
  test "hugo-minimal builds equivalently on claude and pi", %{cwd_for: cwd_for} do
    results =
      Map.new(@harnesses, fn harness ->
        cwd = cwd_for.()
        commits_before = Fixtures.count_commits!(cwd)
        ...
        {exit_code, output} =
          Fixtures.run_codegen_build_parity(
            cwd,
            harness,
            "Create a minimal Hugo site with one post about cats (>= 50 words).",
            stack: "static"
          )

        content_files = Path.wildcard(Path.join(cwd, "content/**/*.md"))
        key_present? = content_files != []
        commits_after = Fixtures.count_commits!(cwd)
    ...
    for {harness, r} <- results do
      if {@scenario, harness} in allowed do
        ...
        assert r.exit == 0, "#{@scenario}/#{harness} exit #{r.exit} (expected 0)\n#{r.output}"
        assert r.key, "#{@scenario}/#{harness} missing content/**/*.md"
        assert r.new_commits >= 1, "#{@scenario}/#{harness} produced no commit beyond scaffold init"
      end
    end
  end
