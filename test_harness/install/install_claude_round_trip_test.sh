...
# NOTE: exclude dotfiles (-name '[!.]*.sh') to align this count with
# install.sh's actual copy semantics — install.sh's copy loop uses a bare
# bash glob (*.sh), which never matches dotfiles, while `find -name '*.sh'`
# matches them regardless of leading dot. No hook test writes a fixture into
# this source dir any more (fixtures live under mktemp -d, symlinked back to
# lib/ for relative sourcing — see no-cat-pipe_test.sh / pre-commit-guard_test.sh),
# and run-tests.sh asserts post-suite that the hooks dir carries zero
# dotfiles, so this filter is a pure semantic alignment with install.sh, not
# a workaround for transient pollution. A single sample is sufficient.
