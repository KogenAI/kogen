...
with tempfile.TemporaryDirectory(dir=out,prefix='native-owned-') as t:
 agents=Path(t)/'.codex/agents';agents.mkdir(parents=True)
 for kind in ['explorer','worker','default']:
  (agents/(kind+'.toml')).write_text('name = "'+kind+'"\ndescription = "Bounded capability probe role"\ndeveloper_instructions = "Return OK without tools."\n')
...
(out/'custom-agent-results.json').write_text(json.dumps(summary,indent=2)+'\n');print(json.dumps(summary,indent=2))
