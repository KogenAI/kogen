defmodule Kogen.Codex.Compatibility do
  ...
  defp invoke_harness(function, arguments, receipt, invocation) do
    ...
    case result do
      ...
      {:error, reason} -> {:error, {:compatibility_turn, function, reason}}
      other -> {:error, {:compatibility_turn, function, other}}
    end
  rescue
  ...
  @doc false
  @spec native_denial_receipt?(map(), String.t(), String.t()) :: boolean()
  def native_denial_receipt?(receipt, invocation, session_id)
    ...
    with %{
      ...
      Enum.any?(String.split(stderr, ~r/\R/, trim: true), &native_denial_line?/1)
    else
      _ -> false
    end
  end
  ...
  defp native_denial_line?(line) do
  ...
  defp blocked_gate?(%{session_id: session_id, bounded_receipt: receipt, bounded_invocation: invocation}),
    do: native_denial_receipt?(receipt, invocation, session_id)
  ...
  defp bounded(context) do
    %{
      context
      | executable: Path.join([priv_dir(), "compatibility", "bounded_exec.py"]),
        args: [context.executable | context.args]
    }
  end
