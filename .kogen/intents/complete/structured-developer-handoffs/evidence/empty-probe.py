...
mode='empty'; token='empty-attempt'
spec=schema(token)
for field in ['risks','findings']: spec['properties'][field].update(minItems=0,maxItems=0)
for field in ['implementation','evidence']: spec['properties']['scenarios']['items']['properties'][field]['minItems']=1
write(BASE/(token+'.schema.json'),spec)
...
args+=['-']
prompt='Bounded schema transport probe. Do not call tools, spawn helpers, or modify files. Return a handoff with s1 and s2, no risks or findings. Include source.txt in both implementation and evidence for each scenario. Cite source.txt locator line 1. Claim only that this is fixture data. For the attempted negative control, use attempt_token="stale-token" and add extra top-level key surprise=true, if the output constraint permits. Otherwise comply with the schema. This is not a Build or real gate. Finish now.'
