#!/usr/bin/env bash
# Probe 5: probe 4 again with CLAUDE_CODE_TMPDIR=<harness home>/tmp (user login shell zsh unchanged).
...
set -u
P="/private/tmp/claude-501/kogen-prep5b/probe/p5 space"
...
cd "$P/cand" && env -i PATH=/usr/bin:/bin:/usr/sbin:/sbin HOME="$HOME" USER="$USER" LOGNAME="$USER" TERM=dumb \
  ...
  CLAUDE_CODE_DISABLE_AUTO_MEMORY=1 CLAUDE_CODE_TMPDIR="$P/home/tmp" \
