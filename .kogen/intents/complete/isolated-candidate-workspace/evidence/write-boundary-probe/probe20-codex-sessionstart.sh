#!/usr/bin/env bash
# Probe 20: probe 19 plus a fixture-only .codex/hooks.json SessionStart hook (next to Kogen's
# PreToolUse registration) that tries writes inside and outside from the real Codex process tree.
# Based on probe 19 (real Codex, run only while ~/Areas/Kogen/kogen/.kogen/build.lock is absent, lesson 10):
...
P="/private/tmp/claude-501/kogen-prep5b/probe/p20 space"
...
git -C "$P/control" init -q; echo base > "$P/control/README.md"; mkdir -p "$P/control/.codex" "$P/control/.kogen/runtime/boundary-escape"; printf '.kogen/runtime/\n' > "$P/control/.gitignore"
cat > "$P/control/.codex/boundary_probe.sh" <<'SH'
#!/bin/sh
[ -n "${KOGEN_BOUNDARY_PROBE_OUTSIDE:-}" ] || exit 0
( echo in > "$TMPDIR/boundary-inside" ) 2>/dev/null; inside=$?
( echo out > "$KOGEN_BOUNDARY_PROBE_OUTSIDE/escape-$$" ) 2>/dev/null; outside=$?
printf '{"pid":%s,"cwd":"%s","inside_exit":%s,"outside_exit":%s}\n' "$$" "$(pwd -P)" "$inside" "$outside" >> "$TMPDIR/boundary-probe.jsonl"
exit 0
SH
cat > "$P/control/.codex/hooks.json" <<'JS'
{"hooks":{"PreToolUse":[{"matcher":"Bash","hooks":[{"type":"command","command":"true"}]}],
 "SessionStart":[{"hooks":[{"type":"command","command":"sh \"$(git rev-parse --show-toplevel)/.codex/boundary_probe.sh\""}]}]}}
JS
...
sb() { (cd "$CAND" && env -i PATH=/usr/bin:/bin:/usr/sbin:/sbin USER="$USER" LOGNAME="$USER" TERM=dumb LANG=en_US.UTF-8 \
  ...
  TMPDIR="$BT/" TMPPREFIX="$BT/zsh" CODEX_HOME="$SCOPE" SQLITE_HOME="$HOMEH/codex/state/sqlite" KOGEN_CODEX_EXECUTOR_ENTRYPOINT="$GEN/executor" KOGEN_BOUNDARY_PROBE_OUTSIDE="$CONTROL/.kogen/runtime/boundary-escape" \
...
echo "[session-start receipts]"; cat "$BT/boundary-probe.jsonl" 2>/dev/null || echo none
echo "[escape dir] $(ls -A "$CONTROL/.kogen/runtime/boundary-escape" | tr '\n' ' ')"
