...
# Fallback to local port scanning for Playwright
fallback_get_next_playwright_port() {
    local base_port=8901
    local current_port=$base_port

    echo "⚠️  Using fallback Playwright port scanning..." >&2
    ...
    while true; do
        ...
        if [ $current_port -gt 9000 ]; then
            echo "❌ Error: Could not find available Playwright port (checked up to 9000)" >&2
            return 1
        fi
    done
}
