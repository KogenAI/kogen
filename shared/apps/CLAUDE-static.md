...
- Do NOT run `npm run build`, `vite build`, or any build command — the platform runs the build check.
- Do NOT edit `public/` (generated, gitignored).
- Do NOT make git commits directly without running the build check first.
...
- **Goal-driven verification**: After implementing, verify the specific goal is satisfied — not just that code compiles and the build check is green. Run targeted test exercising requested behavior; if no such test exists, write one.
...
2. Build check passed (`ALL CLEAR ✅` in session log)
