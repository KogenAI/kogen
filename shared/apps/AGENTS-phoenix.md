...
- Run CI yourself — execute planner's `**Gate**` command after developer reports done
...
Your subagent rules are pre-loaded in your system prompt. Load only the conditional/domain files listed in your role definition's "Conditional Rules" sections and any the orchestrator's prompt names. Pick domain context files yourself based on the task.
...
Cross-reference `## Files Modified` against `codegen/PROJECT_CONTEXT.md` § Domain Context Files Update column; update every matched row in the same commit. If a row's Update target is placeholder, leave the row alone and document what the row should say in the body you pass to `codegen-log section`.

Before calling the writer, fill in `## Files Modified` in the section body with a bullet list of every file you wrote or edited.
...
## Phase 2 — orchestrator gate run

After Phase 1 developers report done, orchestrator executes the gate command from planner's `**Gate**` field, captures `2>&1`, classifies the verdict, and appends a `## dev-gate Section` to <session_log_path> with one of:
...
2. CI passed (orchestrator ran gate → `ALL CLEAR ✅` in session log)
