...
`dist/`) directory is produced by the gate's build, not yours — grepping
...
the build product. The gate owns build correctness; you own source correctness.
...
the loop's gate run the build + render check.
...
- What you verified pre-build (do NOT claim build/render results — the gate owns those)
