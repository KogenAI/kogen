...
**Function-level seams for DB-dependent code in `async: false` tests**: When a private function calls `Ecto.Migrator.migrations/2` or other DB operations and you want hermetic unit testing without DataCase sandbox setup, inject an `:opts` seam at the function boundary with keys for dependencies (e.g., `:pending_fun` → `fn repo, paths -> :ok end` that stubs the migrator call, or `:scan_fun` → `fn opts -> %ScanResult{} end` that stubs the entire scan). Default the opts seam to the real implementation (e.g., `:pending_fun` defaults to `&Ecto.Migrator.migrations/2`). This allows tests to pass a hermetic version that reads from a temp migrations dir or returns a fixture without requiring a live DB connection. Context: `Combobulate.Release.MigrationSafety` test hermetic via `:pending_fun` seam; `Release.migrate/0` boot gate tested with `:scan_fun` seam.

**`on_exit` callbacks run in ExUnit.OnExitHandler process after test process exits** — `self()` inside `on_exit` is NOT the original test PID. Any `Sandbox.allow(Repo, test_pid, self())` inside `on_exit` fails when `test_pid` has already terminated (sandbox connection reverts to `:manual` on owner exit). When a `handle_call` on a global GenServer threads `caller: elem(from, 0)` into Repo calls, the `caller:` routes ownership lookup via that PID, not the GenServer's own. In `on_exit`, the caller is the on_exit runner → no sandbox connection → `DBConnection.OwnershipError`. Fix: if cleanup is unnecessary (unique keys + sandbox rollback already isolate), delete the `on_exit` block entirely. Example: `build_worker_scanners_test.exs` GumroadScanner describe block used `System.unique_integer/1` per test (no map-key collision) + DataCase sandbox (DB rollback automatic) → the `on_exit` clear_pending was redundant.

**Synchronous write-through + caller threading in global GenServers for async tests** — When a global GenServer's `handle_call` does a Repo write and must participate in a test's sandbox connection, thread `caller: elem(from, 0)` (the test PID) into the Repo call options. DataCase sandbox allow grants the GenServer's own PID access but the `caller:` option overrides — Ecto resolves ownership from `caller:`, not the GenServer. Example: `Repo.insert(changeset, caller: elem(from, 0))`. If the GenServer also runs code in `on_exit` (bad practice), the `caller:` will be the on_exit runner (dead process) → delete the `on_exit` cleanup if unnecessary.

## Misc

- **DateTime.utc_now(:second) produces second-precision natively** — no `DateTime.truncate(dt, :second)` needed. When Ecto schema declares `:utc_datetime` (rejects microseconds in insertion), the second-precision is satisfied upstream. Use `DateTime.utc_now(:second)` directly; the `:second` unit argument handles the truncation at the DateTime generation layer.

- **`require Logger` ordering in strict layouts** — `StrictModuleLayout` canonical order is `use` → `import` → `require` → `alias`. `require Logger` must precede `@behaviour` declaration (both are module-level directives; `require` is structural). Credo warns if `@moduletag` appears between `use`/`import` and `alias`/`require` (triggers two violations: one for early `@moduletag`, one for late `alias`). When `require Logger` is present, verify no `@moduletag` sits between `use`/`import` and the `require` line. Project `.credo.exs` may relax strict layout enforcement — if gate passes, deviation is accepted.

- **`Credo.Check.Refactor.FilterIntoWith` + `SinglePipe` bidirectional interaction** — `SinglePipe` fires in two directions: (a) `Repo.one(from(...))` → "use pipe instead"; (b) `from(...) |> Repo.one()` → "pipe chain must not start with fn call". Both can fire on the same code from different angles. Fix: assign the query to a variable, then call `Repo.one(query)` — satisfies both checks simultaneously. Example for DateTime: `DateTime.utc_now(:second) |> DateTime.add(offset, :second)` triggers both `FilterIntoWith` (suggests `|>` over nested) AND `SinglePipe` (warns single-pipe must not start with fn call); use `DateTime.add(DateTime.utc_now(:second), offset, :second)` instead. For Ecto queries: `from(u in User, where: u.id == ^id) |> Repo.one()` triggers `SinglePipe`; use `query = from(u in User, where: u.id == ^id); Repo.one(query)` to satisfy both checks.
...
- **`IO.puts/1` flagged by Credo — use `IO.write(:stdio, msg <> "\n")`**: Credo `Consistency.NoIO.puts` warns on `IO.puts` calls. For CLI entrypoints that must emit output (e.g. `MigrationSafety.cli_report/0`), use `IO.write(:stdio, msg <> "\n")` instead. This satisfies Credo while maintaining the same stdout write semantics.

- **`mix run --no-start` skips `Application.start` → OTP processes unavailable**: When a `mix run -e` invocation needs to access Ecto repos, GenServers, or other OTP processes, omit the `--no-start` flag. `--no-start` skips `Application.start` for the project and all deps — any code path calling an Ecto repo (`Ecto.Migrator.migrations/2`), GenServer, or process raises `:noproc`. Use bare `mix run -e` when the invoked function needs DB access. Context: established pattern in `bin/predeploy` Phase F uses bare `mix run -e` to invoke code paths that require the application to be running.
...
- **Credo `TypeDuplication` → introduce semantic type aliases**: When a type like `String.t()` appears 2+ times across `@type` declarations and `@spec` return types combined, Credo flags `TypeDuplication`. Fix: introduce a semantic type alias (e.g., `@type report :: String.t()`) and reference it in specs. This satisfies Credo while improving code clarity and enabling future refactoring. Example: a `summary/1` function that returns a human-readable report string should use `@type report :: String.t()` + `@spec summary(t()) :: report()` instead of inlining `String.t()` in the spec.

- **Credo `PrivateFunctionSpec` (defp with @spec)**: `@spec` annotations belong ONLY on public functions (`::`). Credo flags `@spec` on `defp` functions as a violation and must be removed. Private functions do not participate in the public API contract and should not carry specs. When tests need to call private fns, use opts seams (keyword args) to inject hermetic stubs rather than writing specs for the private fns. Example: remove `@spec stripe_finding(...)` from a private `defp stripe_finding/1`; keep `@spec check(opts()) :: t()` and `@spec summary(t()) :: report()` on public fns. This ensures the spec surface reflects the public API only.

- **Credo `UnusedType` (type declared but not in any @spec)**: Credo flags `@type` declarations that do NOT appear in at least one public `@spec` line as unused and must be removed. Only define `@type` for semantic concepts actually used in public function specs. Seam types used only in private-fn opts (not in public specs) should NOT be declared as module `@type` — instead, pass their structure inline or rely on the caller's type inference. Example: a module-local seam type `@type jwt_error :: atom()` used only inside `check/1`'s opts handling is unnecessary; remove it. If the seam requires naming for clarity in opts documentation (e.g., `:jwt_fun` comment), name it in the doc string, not as a `@type`.
...
**`Application.compile_env` with a non-test default creates compile-time constant branches** — e.g., `@env Application.compile_env(:app, :env, :dev)` + `if @env == :test` makes the `else` arm unreachable in non-test builds (the condition is `false` at compile time) → Dialyzer warns `pattern_match` (dead false pattern) + `unused_fun` (function reachable only via dead branch). When a test-gate pattern IS intentional (e.g., skip expensive init in test), suppress Dialyzer via `@dialyzer {:nowarn_function, fn: arity}` on BOTH the caller AND the callee (if the callee is reachable only via the gated branch). Note: `@dialyzer {:nowarn_function, fn: arity}` suppresses warnings ABOUT code inside the function (unused params), NOT warnings that the function itself is never called — the latter is `unused_fun` class. Example: `@dialyzer {:nowarn_function, load_fresh: 0}` with `load_fresh/0` unreachable in non-test builds suppresses `unused_fun: load_fresh/0 will never be called`. The pattern requires BOTH annotations: caller's annotation suppresses the `pattern_match` warning on the dead condition; callee's annotation suppresses the `unused_fun` warning on the unreachable function.
...
**Credo `UnusedVariableNames` cross-file stale-beam artifact**: When full-project `credo --strict` flags a variable in a line that single-file `credo --strict lib/path/to/file.ex` passes, the violation is a stale `.beam` artifact. The check is cross-file consistency — Credo compares variable naming across the entire compiled codebase; if an old `.beam` from a prior compile is present, it sees outdated source and flags a line that is actually correct in current code. Symptom: single-file Credo clean, full-project Credo failure on the same line. Cure: `mix clean` to delete all `.beam` files, then re-run the gate. The fresh recompile allows Credo to inspect current source against current binaries — no inconsistency. Not a source code bug; not a failing test — the fix is a rebuild, not a code change.
...
**`Regex.run/3` with `capture: :all_but_first` for bounded clause extraction**: When detecting a pattern in a multi-clause function body (e.g., extracting the `def down` clause from a migration file to check for `raise`), use `Regex.run(~r/def\s+down\b(.*)$/ms, body, capture: :all_but_first)` to extract the matched clause without greedy `.*?` spanning across multiple function definitions. The `capture: :all_but_first` option returns the captured group (the clause body), bounded by the regex anchor. This avoids false positives where a greedy pattern like `def\s+down\b.*?raise\b` crosses function boundaries and matches `raise` in an unrelated helper function below. Context: detecting irreversible migrations where the `down` clause explicitly raises rather than providing a rollback.
