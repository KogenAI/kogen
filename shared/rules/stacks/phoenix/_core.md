...
- **`Credo.Check.Refactor.FilterIntoWith` + `SinglePipe` interaction in test DateTime expressions** — `DateTime.utc_now(:second) |> DateTime.add(offset, :second)` triggers both `FilterIntoWith` (suggests `|>` over nested call) AND `SinglePipe` (warns single-pipe must not start with fn call). Use `DateTime.add(DateTime.utc_now(:second), offset, :second)` (two fn calls, no pipe) to satisfy both checks.
