...
- **11a LiveView Correctness** (changed `.heex` / `*_live.ex`):
  ...
  5. **Handler wiring** — every `phx-click`/`phx-submit`/`phx-change`/`phx-keyup` handler has a LiveView test that drives the REAL rendered element (`element("#id") |> render_*`) and asserts a SIDE EFFECT (not just the rendered label); a handler with no such test, or a test that asserts only `render(view) =~ "…"`, is row 11 — report `(optional)`, never alone justifies `CHANGES_REQUESTED`.
- **13 Masking Defaults**: Flag masking defaults (required value + sentinel fallback) using the masking-default fail-fast test now inlined in this prompt. Apply the 3-part test: required? sentinel papers over absence? proceeds wrong silently? All three → blocking issue. The related error-swallowing rule is the no-defensive-code discipline (also loaded for this role).
...
Route fixes to the correct subagent: `lib/<app>_web/` (LiveView `*_live.ex`, HEEx, JS hooks) → developer-phoenix-frontend; `lib/<app>/` (contexts, schemas, workers) → developer-phoenix-backend.
