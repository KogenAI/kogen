...
A clear verdict is only a license to commit **for the tree it graded**. The loop re-gates automatically right before you run (comparing the gate's stamped `graded_tree_sha` against the tree now) and re-verifies your commit's tree against that value after — practically: commit exactly what's on disk (`git add -A && git commit`), never hand-pick a subset, never let anything mutate the tree between gate and commit.
...
Subject only. No body, ever. The diff shows what changed; the subject names why. `ev:waiver` is durable in the cycle log — never a trailer; name it in the subject if that's the purpose.
...
- Session logs (`codegen/logging/`) are gitignored — NEVER `git add`, `hash-object`, `commit-tree`, `notes add`, or `stash` a cycle log onto ANY branch/ref/note/stash, and NEVER build a wrapper to carry logs across boxes. `git add -A` already skips them. No replacement cross-box transport is planned.
...
When a `git commit` is **DENIED** by a PreToolUse hook, the deny text is returned as the Bash tool result and names the hook + reason. A denied commit never ran — NEVER treat it as committed, NEVER report `status: success`. Classify the deny before retrying:

- **A factcheck or index-parity violation reaches you** (e.g. a stale backtick'd path, `<!-- count: CMD -->NNN` mismatch, or missing `PROJECT_CONTEXT.md` row): this should never happen — `context-factcheck-edit-gate.sh` denies it in the curator's own turn, and the in-loop scan seeds it into the curator's first prompt plus re-scans post-turn before the committer runs. If you see one anyway, it is outside your write scope (you do not `Edit context/*.md`). Report `status: failed`, quoting the deny's `<file>:<line>` and claim verbatim, so the loop routes it back to curator/developer. Do NOT hand-edit the doc yourself.
