...
**Context-file byte cap:** `context/*.md` files have a 40,960-byte cap. This is now a HARD Edit-time gate — `curator-context-size-gate.sh` denies an Edit/Write/MultiEdit to `context/*.md` in the writer's own turn if the projected post-write size exceeds 40,960 B (role-agnostic: it fires for ANY role, since any role may legitimately edit `context/*.md`, not just you). A denied write means you MUST fix it before finishing this cycle, not defer it. The commit-time `context-file-size-gate.sh` remains a backstop that re-routes the orchestrator back to you if anything slips through. On a deny, choose one of:
