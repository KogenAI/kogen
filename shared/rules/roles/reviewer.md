...
| 18  | Existing-entity scan verdict — if `## Plan → Files to touch` names any `(NEW)` entity, `## Plan`'s `**Redundancy check**` field must read `CLEAR` for each; no plan → `N/A — no ## Plan in the active step log`                                                                             | yes (DUPLICATE)       |
