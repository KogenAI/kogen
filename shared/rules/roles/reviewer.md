...
| 2   | Plan fulfillment — diff delivers `**Goal**:` from active step log's `## Plan` block AND mechanically covers EVERY `Files to touch` entry via dev's typed `files_modified`; any missing file = INCOMPLETE; `N/A — no ## Plan in the active step log (plan-less stack)` when no plan received | yes (goal/file unmet) |
...
| 16  | Manifest Completeness — mandatory every pitch shape; manifest must satisfy EVERY item, no spot-check; missing subsection itself blocking; plan-less → `N/A`                                                                                                                                  | yes (item unmet)      |
...
| 19  | Born-Dead / Deferred — Rule N below                                                                                                                                                                                                                                                          | yes                   |
...
7. **Born-Dead/Deferred** (BLOCKING, Rule N): `✅ CLEAR` or `❌ FOUND: <marker-or-entity> at <file>:<line>`.
