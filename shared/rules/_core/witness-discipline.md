...
- Producer (`phoenix-dev-gate.sh`): FAILED branches compute `extract_witness "$log_path"` (opaque exit≠0) OR pass the already-known reason (runner-missing, no-op, wiring/render FAIL). Threaded into `gate-result.json` `witness` field + `Witness: <x>.` block-envelope prefix when non-empty.
...
- **Orchestrator**: route the witness verbatim to the next agent; never strip it to a bare "gate failed".
