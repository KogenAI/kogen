...
- **Rule-file line caps are STYLE_GUIDE targets (advisory, for new files); the committed budget is the enforced number** — `_core/` rule files target <50 lines; `roles/` and `stacks/` target <150 lines, but `make prompt-size-budget` (component of `make test`) enforces each file's own row in `templates/generator/prompt-budgets.txt` instead, frozen at CURRENT size (several files already exceed the STYLE_GUIDE target as pre-existing scar tissue). The ceiling is operator-owned: `prompt-budget-writer-only` denies every agent write path to `prompt-budgets.txt` (Edit/Write/MultiEdit, the `--write` flag, Bash write-vocab) — a red verdict means shrink the file or evict the lowest-value content, never raise the cap. `rule-edit-reach.sh` predicts this SAME committed-budget row PLUS the fragment's transitive fan-out across rendered agent prompts, at edit time, role-agnostic, delivered via `hookSpecificOutput.additionalContext` (see `context/hook-authoring-patterns.md` § Hook Output Protocol) — the channel the model actually reads. Only the `context/*.md` / `PROJECT_CONTEXT.md` / `codegen/PROJECT_CONTEXT.md` byte cap (40,960 B) is hook-enforced (denied) at Edit time. **Unit-conflation hazard**: when a function measures two different domains in different units (rule-file budget in lines, rendered-prompt capacity in bytes), never apply a delta parameter (added-bytes) to both outputs interchangeably — bytes and lines are dimensionally distinct, so `current_lines + added_bytes` yields nonsense. Report each domain in its own unit, no projection. **Fail-open advisory hooks**: when a hook emits `hookSpecificOutput.additionalContext` but cannot block, the fail-open design (exiting 0 on unparseable payload, unresolvable repo root, or render error) is legal ONLY if a fail-closed backstop exists elsewhere that catches real overflows — the hook is optional-observability by design, never a substitute for the gate. The test suite must exercise every fail-open branch explicitly rather than leaving silent branches unverified.
