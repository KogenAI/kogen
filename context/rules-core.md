...
- **Rule-file line caps are STYLE_GUIDE targets, enforced as a build-time ceiling, not a per-edit hook** — `_core/` rule files target <50 lines; `roles/` and `stacks/` target <150 lines. `make prompt-size-budget` (component of `make test`) fails when a rule file or rendered agent prompt exceeds its committed ceiling in `templates/generator/prompt-budgets.txt` — this freezes CURRENT size (several files already exceed the STYLE_GUIDE target as pre-existing scar tissue) as a ceiling so growth never happens silently; raising the ceiling is a reviewable diff via `prompt_size_budget.py --write`. `context-curator-guard.sh`'s `warn_if_over_cap` remains the edit-time stderr-only early warning (curator-only, never blocks); the Makefile gate is the enforced backstop. Only `context/*.md` byte cap (40,960 B) is hook-enforced at Edit time.
