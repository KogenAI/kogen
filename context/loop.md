...
| review verdict                                                    | **LLM**-authored, deterministically PARSED — unparseable/ambiguous → `:unknown` → **fails closed** (one reviewer retry, then loud error; never silently advances as approved). Reviewers author one bare terminal `REVIEW_VERDICT: …` line; the parser separately tolerates that exact line wrapped once in a single outer `**…**` pair as a presentation-only recovery boundary — every other wrapper (backticks, headings, code fences, duplicate/conflicting markers, non-terminal placement) stays `:unknown` | reviewer body parse                                                                                                                           |
| the commit + `COMMITTED: <sha>` line                              | **deterministic** — the loop shells `codegen-commit --subject <sealed>` (a script, not a role); no model call, no committer role exists. See pitch "committing is deterministic, not a model call"                                                                                                                                                                                                                                                                                                                | `OrchestrationLoop.run_commit_step/3` → `codegen-commit`                                                                                      |
