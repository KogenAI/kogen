...
| `install.sh`                                  | Per-harness install via `for _harness in "${HARNESSES[@]}"` loop with `case "$_harness" in` dispatch; reads manifest for PATHS, hardcodes its own step order directly (the manifest's `install_steps:` field is declared but read by nothing) |
