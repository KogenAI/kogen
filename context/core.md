...
- `per_subagent: {<subagent_type> => {read_count, edit_count, write_count, bash_count}}` — **claude only**. Joined via `parent_tool_use_id` on nested assistant/user events against the parent `Agent` tool_use's `input.subagent_type` (verified live: a spawned subagent's Bash tool_use carries `parent_tool_use_id` equal to the spawning `Agent` tool_use's `id`). Pi has no subagent-spawn event in its vocabulary — this key is OMITTED on the pi leg, always.
- `stop_reason` — claude only, sourced from the terminal `result` event's `.stop_reason`; OMITTED when absent from the fixture/trace (pi's `agent_end` carries no equivalent field — OMITTED on the pi leg, always)
- `rate_limited` — claude only, `true` when the last `rate_limit_event`'s `.rate_limit_info.status != "allowed"`; OMITTED (not `false`) when no `rate_limit_event` appears in the trace. Pi emits no rate-limit event — OMITTED on the pi leg, always.

Per-harness omission is structural, not a bug: pi's `tool_execution_end` events carry `toolName` (lowercase: `read`/`write`/`bash`/`edit`) and `isError`, but no `parent`/subagent-join key, no rate-limit signal, and the real `agent_end` event itself carries no top-level `usage`/`num_turns` (unlike the pre-existing test fixture, which fakes both — a real live probe confirmed this asymmetry). Every field documented above was captured from a REAL claude/pi stream, never hand-authored — fixtures: `harnesses/claude/hooks/fixtures/tool_trace.jsonl` (Read+Write+Bash, real `rate_limit_event`+`stop_reason`), `tool_trace_subagent.jsonl` (real `Agent`-spawned subagent Bash, `parent_tool_use_id` join), `pi_tool_trace.jsonl` (real pi `tool_execution_start`/`_end` + `agent_end`).
