...
| `codegen-commit`   | Deterministic commit step — `--subject "<text>" [--cwd <dir>] [--no-gate] [--allow-revert]` validates + stages (`git add -A`) + checks gate verdict + runs the backward-roll guard + commits; `--check-subject "<text>"` validates only, no git state touched. Replaces the paid `committer` subagent; no model call. Contract: pitch "committing is deterministic, not a model call".                                                                         | Loop's commit step, `/ready`, operators  |
...
| `codegen-advise`   | Wraps `codegen-call`; maps to a STRONGER tier of the SAME harness (fixed map); `--cwd` REQUIRED. Assembles a bounded, machine-built evidence packet from git/gate state before calling; returns `{diagnosis, falsifier, next_probe, evidence_used, missing_evidence, confidence}`. See `context/loop.md` § Same-Harness Advisor / `context/harnesses.md` § Same-Harness Advisor.                                                                               | Loop give-up boundary, `advise` tool     |
...
| `harnesses/claude/hooks/`                         | 44 hook scripts (PostToolUseFailure, PreToolUse, Stop) + paired `_test.sh` + `lib/` |
...
| `shared/rules/roles/`  | Per-role rules: context-curator, developer, reviewer                                        |
