...
| `codegen/`      | Self-meta session logs for codegen-on-codegen dev. Separate from downstream project `codegen/` dirs. Also holds `codegen-propose` output (`analysis-proposals/`). | `codegen/logging/*.md` (active session logs), `codegen/analysis-proposals/<from>_<to>.md` (proposed-change records, ephemeral) |
...
- [ ] **AGENTS.md and CLAUDE.md at root are hand-authored regular files** — edit them directly; no `.j2` template sources exist and `install.sh` does NOT render them. These are independent guidance files for agents working in THIS codegen repo (distinct from downstream consumer repos' auto-rendered AGENTS.md/CLAUDE.md).
