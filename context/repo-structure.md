...
| `codegen-build`    | Harness API — `--harness`, `[--stack]`, `--cwd`, `--model`, `--effort`, `--max-budget-usd`, `--fallback-model`, `--print-dir`, `--print-argv`, `-h`/`--help`; stack detected from cwd markers (`mix.exs`→phoenix, `vite.config.js`→static) when omitted; delegates to `harnesses/<harness>/dispatch.sh`. `--max-budget-usd=<n>` threads via `CODEGEN_BUILD_MAX_BUDGET_USD` into an explicit `--max-budget-usd=<n>` argv entry to `mix codegen.loop`, enforced between role invocations (an ENFORCED per-cycle spend cap, not a passthrough flag) — absent, no cap. `--print-argv` dry-runs: parses+validates, prints the would-be `mix codegen.loop` argv (including `--max-budget-usd` when set), exits 0, never mutates the target cwd or execs the loop. Public flag surface is pinned by a committed snapshot fixture (`harnesses/claude/hooks/fixtures/codegen-build-flags.txt`) — `make test` fails if the parser's flags drift from it. | Platform, CI, downstream Makefiles                                                |
...
| `codegen-log`      | Sole writer of append-only JSONL cycle logs (`init`/`section`/`append`/`verdict`/`relocate`/`show` subcommands — `show` is a read-only operator projection, writes nothing); see `shared/rules/_core/session-log.md` for full CLI contract                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     | Loop, subagents (planner/dev/reviewer/curator/committer); `show` is operator-only |
