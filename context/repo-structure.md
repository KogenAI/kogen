...
- [ ] **Generated `*-system-prompt.txt` files** — never hand-edit; regenerated by `make install`. These files are gitignored (`.gitignore:35` pattern `*-system-prompt.txt`), NOT committed tracked artifacts — they regenerate on install and are local-machine ephemeral, unlike `shared/apps/*.md` (committed generated files, see § Notable Artifacts). Planners MUST NOT ask developers to commit these system-prompt files.
