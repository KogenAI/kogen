...
**Key fact — AGENTS.md / CLAUDE.md at repo root**: Both are hand-authored plain files describing codegen's own session loop. NOT generated, NOT symlinked — edit directly. `CLAUDE.md` is gitignored (local install artifact; never commit it). `AGENTS.md` is committed. `templates/AGENTS-HYBRID.md.j2` generates a _downstream_ repo's docs (not these root files).
...
├── CLAUDE.md                     ← codegen session loop docs (gitignored local install artifact)
...
| `CLAUDE.md`          | AI sessions (claude render)    | Same content; regenerated by claude harness renderer        | Gitignored; never commit |
...
| `harnesses/claude/hooks/`                         | 47 hook scripts (PreToolUse, SubagentStop, Stop) + paired `_test.sh` + `lib/`            |
...
- [ ] **One-way knowledge boundary**: Codegen docs MUST NOT name any downstream consumer. Use "the consuming app" / "downstream projects". `grep -ri "<consumer-name>" context/ AGENTS.md CLAUDE.md` must return zero hits.
- [ ] **CLAUDE.md is gitignored** — committer MUST NOT stage it. Only `AGENTS.md` is committed.
