...
| `shared/enforcement/seam-registry.yaml`           | Contributors / curator / planner             | Manual edit; `seam-registry-parity_test.sh` checks at `make test` | New/removed declaration↔reflection seam (twin, generated pair, index)                |
...
- **`harnesses/claude/hooks/`** — 50 hook scripts; each enforces one discipline rule, has a paired `_test.sh`. Full catalog: `context/hooks.md`.
...
- **`shared/usage_rules/` vs `shared/recipes/`** — usage rules are machine-generated API summaries; recipes are hand-authored patterns.
...
- **`analysis/proposer/`** — subpackage consumed by `codegen-propose`. `select.py` is pure/deterministic: filters+ranks `codegen-analyze --json` clusters by `COUNTER_CONFIDENCE_PRIOR` × `wasted_turns`, drops `DROP_COUNTERS`/sub-min-turns clusters, caps at `--max`. `__init__.py` carries `COUNTER_FIX_TYPE` (counter → target-file-class). Tests: `analysis/tests/test_proposer.py` (`make test` auto-discovers).
- **`shared/scaffold/SCHEMA_VERSION`** — single-integer scaffold schema version, source of truth for downstream-app staleness detection. `codegen-scaffold` stamps it (plus `scaffolded_from_sha`/`scaffolded_at`) into `<app>/codegen/manifest.yaml` on every `create`/`integrate` (ALWAYS-REWRITE). `codegen-build` refuses (exit 2, fail-closed) when the stamped version is behind current, naming `codegen-scaffold integrate --stack=<s> --cwd=<cwd>` as remediation. `<app>/codegen/` is gitignored.
- **`~/.local/bin/.ocg-install-stamp`** — install-currency stamp written by `install.sh` at the end of every `make install` run (`sha=<HEAD>` / `hash=<working-tree content hash of the install source set>`). Single source of the source-set list + hash formula: `harnesses/shared/build-ready-currency.sh` (sourced by both `install.sh`, the writer, and `make build-ready`, the reader — never duplicate the list). Content hash uses `git hash-object` only (no `sha256sum`/`shasum`, which differ macOS↔Linux) over CURRENT working-tree bytes, so an edited-but-uncommitted source reads as stale even at the same `HEAD`. Machine-local, never committed.
- **`shared/enforcement/seam-registry.yaml`** — canonical inventory of every declaration↔reflection seam (twin files, generated-from-source pairs, indexes). `seam-registry-parity_test.sh` reconciles it against real Makefile targets + hook test files at `make test`: every registry row's `guard` must resolve, every repo guard must be registered and actually run, every `GAP` row must carry a rationale. See the file's own header comment for full field spec.
- **`harnesses/claude/document-system-prompt.md`/`document.schema.json`** — headless drafter, sibling of `propose-system-prompt.md`; behind `LoopQueueDrain`'s `:draft_fn` seam.
