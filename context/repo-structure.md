...
- **`analysis/proposer/`** — subpackage consumed by `codegen-propose`. `select.py` is pure/deterministic: filters+ranks `codegen-analyze --json` clusters by `COUNTER_CONFIDENCE_PRIOR` × `wasted_turns`, drops `DROP_COUNTERS`/sub-min-turns clusters, caps at `--max`. `__init__.py` carries `COUNTER_FIX_TYPE` (counter → target-file-class) consumed by `propose-system-prompt.md`. Tests: `analysis/tests/test_proposer.py` (`make test` auto-discovers).
...
- **`shared/enforcement/seam-registry.yaml`** — canonical inventory of every declaration↔reflection seam (twin files, generated-from-source pairs, indexes) in the repo. `seam-registry-parity_test.sh` reconciles it against real Makefile targets + hook test files (naming-convention scan) at `make test`: every registry row's `guard` must resolve, every repo guard must be registered, every `make-test`-time guard must actually run, every `GAP` row must carry a rationale. See the file's own header comment for full field spec.
