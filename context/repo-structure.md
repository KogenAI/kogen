...
- [ ] **CLAUDE.md and AGENTS.md are both committed symlinks (mode 120000)** — they point at generated `shared/apps/CLAUDE-phoenix.md` and `shared/apps/AGENTS-phoenix.md` respectively; the rendered `.md` targets are gitignored. To change content, edit the `.j2` template and run `make install`. When codegen self-builds, `codegen-build` skips the integrate pre-step to prevent re-symlinking over the committed symlinks.
