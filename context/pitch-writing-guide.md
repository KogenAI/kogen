...
**Each entry must be a real repo-relative path, not a glob.** No glob metacharacter (`*`, `?`), no leading `/` (absolute), no `..` path segment — a glob never string-matches a literal path, so it silently reports as disjoint from a sibling pitch editing the same files, and an absolute or outside-repo path grants a role read/write it cannot use. A malformed entry (glob/absolute/`..`) is a distinct, stronger finding than an entry naming a path that does not exist yet — the latter is legal (a pitch may declare a file it is about to create). The check REPORTS every malformed/missing entry across every pitch in the batch rather than raising and aborting on the first one — one bad pitch's `scope:` must never hide the findings for every pitch after it in the same directory.
