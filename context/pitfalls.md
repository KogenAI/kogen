...
- **[local] Preflight probes must supply all pre-dispatch codegen-call flags even at zero cost** — A turn-0 probe designed to exit before any model turn (e.g., `--agent=sentinel` triggers immediate `exit 1`) must provide `--model` and `--effort` flags to `codegen-call` argv, because `call-dispatch.sh` validates both pre-dispatch in bash. Use static placeholder (e.g. `--model=sonnet --effort=low`); these flags are pre-dispatch validation only, not semantic model config.
...
- **[shared] `Port.open` `:env` requires charlist tuples** — `{:env, [{"KEY","VAL"}]}` raises error; use `{:env, [{~c"KEY", ~c"VAL"}]}`. See `fixtures.ex` + `bench_artifacts.ex` for ref pattern.
- **[local] `Task.shutdown(:brutal_kill)` doesn't kill OS children** — Use `Port.open` + process-group kill instead.
