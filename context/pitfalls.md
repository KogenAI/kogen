...
- **Gate Verdict Authority applies retroactively** — the runtime-written gate-result JSON's `.verdict` field is authoritative, never session-log prose. Timestamp cross-check (gate-result.json timestamp vs session log's `## dev-gate Section` entry) proves THIS gate run vs stale. Quick pre-existing-bug proof: `git log --oneline -- <file>` compares file's last-touch commit vs session Version Stamp commit.
