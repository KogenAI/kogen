...
- **[shared] RED-then-GREEN at source-grep layer for verdict-string flips** — When converting a JS output value (e.g., INCONCLUSIVE:reason → FAIL:reason), the `.sh` gate's generic `FAIL:*` case arm catches all new values without code changes. Source-grep RED-then-GREEN in the test harness (`*_test.sh`) proves the verdict flip — running the test against the PRE-fix source (red) then POST-fix (green) is cleaner than integration-level stub tests that can't distinguish pre/post because the stub maps any FAIL:\* → block generically. Layer the proof where the verdict is produced, not where it's routed.
- **[local] RED-then-GREEN for source-grep guard tests via scratch copies** — When adding a source-grep guard (e.g., `grep -c PATTERN file` must be 0), prove it catches violations without altering the real source: copy the target script to `/tmp`, inject a forbidden token, copy the test to `/tmp` and patch its target-path variable via `python3 -c '...' `, run the test against the injected copy (RED), confirm real source re-passes (GREEN). Isolates violation-detection proof from the clean source; avoids `git stash`/restore cleanup, cheap insurance per repo regression-guard discipline.
...
pitfall, gotcha, bash pattern, yq null safety, JSONL migration, format flip, fixture ripple, gate verdict staleness, partial fix
