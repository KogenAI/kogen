...
- **rules**: `AGENTS-phoenix.md` encodes the downstream-app session loop prose for human-driven interactive sessions in the downstream app; codegen builds always use the deterministic `OrchestrationLoop`, never this session-loop prose — see `context/rules-roles.md` and `context/test-harness.md`
...
## Cache Key Ordering & Honesty

**Cache key must be computed AFTER `mix deps.get`**, not before. When the key is keyed on `mix.lock` before the deps fetch, it only hashes the stock (pre-mutation) lock file — all mutations injecting new deps into `mix.exs` are not yet reflected. Result: every provisioning run reports a false cache "hit" while still cold-fetching every injected dep. The correct pattern is:

1. Run `mix deps.get` (which applies all mutations from the previous phase)
2. Compute cache key against the real, mutation-inclusive lock
3. Restore pre-built artifacts (`deps/`, `_build/`, `dialyzer.plt`) if key matches

Restore must also be all-or-nothing: a failed `cp` on any single file fails the entire restore, falling back to a cold run. Partial restores (a broken `_build` dir) silently produce wrong binaries and are worse than recompiling from scratch. Track restore success per-file, not per-cp-command: if ANY `cp` in the restore sequence fails, the whole restore fails.
...
**Gitkeep/keep sentinels are gone — clone survivability**: `codegen/pitches/{draft,ready,shipped}/` and `priv/plts/` both sit under UNNEGATED `.gitignore` boundaries (`/codegen/`, `/priv/plts/`) — a sentinel file (`.gitkeep`/`.keep`) placed inside them cannot be staged (`git check-ignore -v` names the boundary) and therefore cannot survive a clone. Neither `scaffold.sh` (either stack) nor `codegen-scaffold`'s `run_integrate_stage` write a sentinel any more; `mkdir -p` alone is sufficient because `run_integrate_stage` recreates both dirs on every `create`/`integrate` — the durable, always-runs owner. Scaffold-time `mkdir -p` only covers first provision before any build has run integrate.
...
1. Runs `mise trust` on the generated app's `.mise.toml` (mirrors the guarded `mise trust` block in `install.sh`, search `mise trust`). Tool ABSENCE (`mise` not on PATH) stays a quiet skip; tool FAILURE (`mise` present but trust genuinely fails) is FATAL — an untrusted `.mise.toml` means `.env` never loads locally (`.mise.toml`'s `[env] _.file = '.env'` is the only local `.env` loader).
2. Renders `.env` with a REAL `SECRET_KEY_BASE` generated by `mix phx.gen.secret` run inside `$TARGET_DIR` (the task only exists once `mix phx.new` has produced a real Mix project there — every other mix/npx/git call in `render()`'s neighbourhood already wraps `(cd "$TARGET_DIR" && ...)`; secret gen matches that form). Failure is FATAL, never a silent placeholder substitution — plus DB var (unless `--no-ecto`; `DATABASE_URL` for postgres/mysql/mssql, `DATABASE_PATH` for sqlite3 — see `--database` passthrough above), `PHX_HOST`, `PORT`
...
- **Key**: `<otp>-<elixir>-<sha256(mix.lock)>` — `$OTP_VERSION`/`$ELIXIR_VERSION` scaffold vars (not a runtime `elixir --version` probe) + a sha256 hash of the generated app's `mix.lock`. **Computed AFTER `mix deps.get`, not before** — mutations (Phase 2) inject optimum deps into `mix.exs`, but `mix.lock` is not updated until `deps.get` resolves it. Computing the key from the pre-`deps.get` `mix.lock` (the previous ordering) hashed the STOCK `phx.new` lock, so every scaffolded app's key ignored the injected deps entirely — a printed "cache hit" masked a cold `deps.get` fetch happening right after it. `deps.get` now runs unconditionally FIRST; the key and restore attempt come after, against the real lock the app builds against. This restore only saves compiled-artifact time (`_build/*/lib/<dep>`, PLT) — `deps.get` itself is never skippable by this cache.
...
- **Restore** (Phase 5, immediately after `mix deps.get`): on a hit, `cp -R` copies the three layers into the target dir; `mix setup` still runs unconditionally afterward (cheap no-op when already warm; it validates the lock and catches drift). **All-or-nothing**: any individual `cp` failure fails the WHOLE restore (not just that one artifact) and removes whatever the call already copied, so a fallback to cold never builds on top of a half-restored `deps/`/`_build`/PLT reported as a hit.
