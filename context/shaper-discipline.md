...
| I | Ask-vs-decide classifier | Ask the user ONLY when the answer changes what they experience or names something they own. Test: "Does the answer change what the user experiences or names something the user owns?" If no → auto-decide from code + convention. Engineering-completeness (install-guarantee, fail-closed-when-guaranteed, internal naming, split mechanics) is NOT a user decision → auto-decide and record as `Assumed: <dimension> = <default> (override if wrong)`. |
