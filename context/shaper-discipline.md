...
**Grammar**: `blocks_on:` is a YAML frontmatter flow-list of bare slugs, e.g. `blocks_on: [dep-one, dep-two]`; empty/independent is `blocks_on: []`. **Dual-read**: pitches with no frontmatter block fall back to the legacy prose grammar — a `## Dependencies` block body of zero-or-more `Blocks-on: <slug>` lines (one slug per line), also accepted inside `## Related pitches`. New pitches should emit frontmatter; the legacy grammar remains parseable for existing files.
