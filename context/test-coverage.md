...
| session-log-writer-only          | hook           | `.../hooks/session-log-writer-only.sh`          | `.../hooks/session-log-writer-only_test.sh`          |                                              |
...
- Pi build dispatch parity now has a dedicated hermetic shell test (`harnesses/pi/dispatch_test.sh`) that checks the generated prompt + default build extensions + additive caller extensions.
