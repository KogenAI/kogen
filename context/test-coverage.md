...
| codegen-call entry     | launcher   | `codegen-call` (repo root)                   | `harnesses/claude/hooks/codegen-call_test.sh` (via `make test`); `test_harness/test/codegen_test_harness/call_contract_test.exs` (`--role` rejection + real-caller contract)                                | One-shot structured LLM call binary                                            |
| codegen-document entry | launcher   | `codegen-document` (repo root)               | `test_harness/test/codegen_test_harness/call_contract_test.exs` (no real caller passes `--role`; `SYSTEM_PROMPT_FILE` target exists + is `.md`; whole-lock `main()` returns non-zero on `failed_rules > 0`) | Usage-rules doc generator for hex deps                                         |
...
| scaffold_cache.sh   | mutation-helper | `shared/scaffold/phoenix/scaffold_cache.sh`          | `shared/scaffold/phoenix/scaffold_cache_test.sh` (26 cases: key determinism/sensitivity, restore hit/miss/disabled/all-or-nothing-partial-failure, save inclusion/exclusion/atomicity) | Machine-global deps/\_build/PLT cache; sourced by phoenix scaffold.sh             |
