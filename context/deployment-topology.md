...
**A ready pitch with a local recovery dossier refuses cross-node assignment** (see pitch "restarted builds resume owned work" and `context/loop.md` § Interrupted-Cycle Recovery). `refuse_if_recovery_bound` runs right after the same-node no-op check (so same-node placement is still a pure no-op) but BEFORE `dependency_preflight` and any transfer, for both the local-fs and remote legs of `cmd_assign_one`. It enumerates `codegen/gate-pending/recoveries/<slug>/*.json` on the SOURCE node: any regular file present, or an unreadable directory (fail-closed — never silently "safe to move"), exits 1 naming `recovery-bound <slug>`. `--auto` reaches the same predicate through its per-lane `cmd_assign_one` call — an earlier normal lane still places under `--auto`'s existing partial-progress contract (fail-closed stops the run only at the recovery-bound lane itself), but that lane's own slug never moves. The dossier JSON and its immutable Git ref are machine-local state pitch-file transfer does not carry; a downstream fleet feature may later add atomic dossier+ref transfer, but until then this refusal is the safe interim.
