...
retry-eligible. This is what makes a self-inflicted, deterministic exhaustion (a gate or env-var check
the owning role genuinely could not fix) stop costing a full `codegen-build` price on every retry
instead of failing once. The two orientation-doc exhaustions
(`curator_doc_check_exhausted/3`, `turn0_repair_exhausted/3`) deliberately write NO marker — doc drift is
a retryable cycle failure, not a permanent one, and marking it would park a pitch a second curator pass
would have cleared. Absent or malformed marker (`:absent`) →
