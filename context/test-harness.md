...
- `LoopQueue` mirrors `retryable_regex` transient classification and Kahn topo-sort. `LoopQueueDrain.drain/1` is its live caller, gated like the one-shot build: `--elixir --queue` execs `mix codegen.loop.queue`. Bare `--queue` (default) execs legacy `harnesses/shared/build-queue.sh` directly (not via dispatch.sh's exec-env), reading its own per-pitch budget toggles from ambient env. Both paths move via `File.rename!`; no double-move (disjoint invocation paths).
