...
- **Telemetry** — `accumulate_telemetry/3` (`/2` delegates) sums `usage` into a process-dict accumulator per role incl. retries; 3rd arg = requested `{harness,model,effort}` dispatch, read via `UsageParser.parse_dispatches/1`. `Mix.Tasks.Codegen.Loop.emit_loop_telemetry/1` prints one aggregated line after `run/1`. Campaign binding → `context/loop.md`.
- **Gate selection** (`LoopGate.decide_gate/1`, `gate-select.sh`) — 2-source tree: (1) planner Tier-1 `**Gate**:` block, wins over (2) per-app `.claude/gate-config.sh` `GATE_COMMAND=<cmd>`. Falls back fail-loud `__GATE_UNRESOLVED__`. Static tests MUST supply GATE_COMMAND or planner step_log; no stack-default fallback.
