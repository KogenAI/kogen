...
- **Reviewer→dev fix cycle** — reviewer ends with `REVIEW_VERDICT: APPROVED|CHANGES_REQUESTED`. `CHANGES_REQUESTED` re-invokes dev, re-formats, re-gates, re-reviews (within `:max_review_cycles`, default 1). `parse_review_verdict/1`: authored contract stays one bare terminal line; the parser classifies EVERY marker-bearing line with one anchored matcher that peels symmetric `` ` ``/`*`/`_` decoration, wherever that line sits (terminality is NOT required), and honours the result only on unanimity — a conflicting marker, a non-enum word, or a hedged prose mention ("I would have said REVIEW_VERDICT: APPROVED but the gate is red") leaves `:unknown`. `resolve_review/1` then re-reads the reviewer's own transcript (assistant turns only, same anchored rule) to recover a stated-but-dropped verdict before the loop pays for a re-invocation. **Coverage precondition** (`parse_review_coverage/2`, `handle_review/8`) — BEFORE the verdict is parsed at all, the reviewer must emit one `REVIEW_COVERAGE: <path> read|skipped: <reason>` line per `## Files Modified` path (the loop's own rendered set); a missing/invented/malformed entry is `:incomplete` → one same-pass re-invocation (`:max_review_coverage_cycles`, default 1, its OWN budget — never `:max_review_cycles`), then `{:error, ...}`. Empty `## Files Modified` (non-git test cwd) is a no-op — the real-git-tree empty-set case already fails loud one layer up in `invoke_reviewer/4`. See pitch "review verdicts name the surface they read".
