...
- **Owner-routing/flake/marker seams** — `:gate_classify_fn` is 2-arity now (`(cwd, dev_role) -> {:owner, role} | :infra`, was 1-arity `:code | :infra`). `:flake_check_fn` (default: `:gate_fn`) backs the one flake re-run before rework. Drain's `:terminal_marker_fn` reads `terminal-state.json` — see loop/loop-queue-drain owner files.
