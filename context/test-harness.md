...
| `test_harness/lib/mix/tasks/codegen.loop.ex`                  | `mix codegen.loop --harness=claude_code --stack=<phoenix\|static> --cwd=<dir> <pitch>` — entrypoint `dispatch.sh`'s build path execs.                                             |
| `test_harness/lib/mix/tasks/codegen.loop.queue.ex`            | `mix codegen.loop.queue --harness=claude --stack=<S> --cwd=<dir>` — wraps `LoopQueueDrain.drain/1`; invoked by `--queue` launcher flags. |
...
| `test_harness/test/stacks/modes/debug_test.exs`       | Asserts debug launcher emits diagnostic report + writes no files                                | Debug (claude)          |
| `test_harness/test/stacks/modes/shape_test.exs`       | Asserts shape launcher produces/edits draft pitch with Shape Up sections                        | Shape (claude)          |
...
- **Per-role harness** — `RoleResolver.resolve_harness/2` reads optional `.harness.<role>.harness` in `config.yaml`, else build's `--harness`. Resolved at 3 chokepoints: `invoke_role/4`, `maybe_escalate_model/5`, `handle_switch_model_failure/7`.
- **Reviewer→dev fix cycle** — reviewer ends with `REVIEW_VERDICT: APPROVED|CHANGES_REQUESTED`. `CHANGES_REQUESTED` re-invokes dev, re-formats, re-gates, re-reviews (within `:max_review_cycles`, default 3). `parse_review_verdict/1`: authored contract stays one bare terminal line; the parser classifies EVERY marker-bearing line with one anchored matcher that peels symmetric `` ` ``/`*`/`_` decoration, wherever that line sits (terminality is NOT required), and honours the result only on unanimity — a conflicting marker, a non-enum word, or a hedged prose mention ("I would have said REVIEW_VERDICT: APPROVED but the gate is red") leaves `:unknown`. `resolve_review/1` then re-reads the reviewer's own transcript (assistant turns only, same anchored rule) to recover a stated-but-dropped verdict before the loop pays for a re-invocation. **Coverage precondition** (`parse_review_coverage/2`, `handle_review/8`) — BEFORE the verdict is parsed at all, the reviewer must emit one `REVIEW_COVERAGE: <path> read|skipped: <reason>` line per `## Files Modified` path (the loop's own rendered set); a returned sign-off value can recover coverage from the reviewer's assistant transcript turns, but a still-missing/invented/malformed entry is `:incomplete` → same-pass re-invocation (`:max_review_coverage_cycles`, default 2, its OWN budget — never `:max_review_cycles`), then `{:error, ...}`. Empty `## Files Modified` (non-git test cwd) is a no-op — the real-git-tree empty-set case already fails loud one layer up in `invoke_reviewer/4`. See pitch "review verdicts name the surface they read".
- **Review rework routing** — a blocking verdict resolves developer from the configured stack, not historical artifacts, so a resumed GATED cycle reworks and re-reviews. At the final re-review, escalation/advice target the reviewer. A `CHANGES_REQUESTED` whose every finding is `Non-blocking:` proceeds without rework but passes its text to `context-curator`; coverage transcript recovery and its separate budget remain unchanged.
...
- **`call-dispatch.sh` flip** — `codegen-call --agent <role>` exports `CODEGEN_CALL_AGENT`; `harnesses/claude/call-dispatch.sh` reads it, appends `--agent "$AGENT"`, OMITS `--append-system-prompt` (native agent identity replaces it).
...
- `make test-stacks-claude` → `mix test --only slow` — only runs LLM-dependent tests tagged `:slow`
...
When testing launcher logic (e.g., cwd normalization in the build launcher `claude-build.sh`), stub the underlying binary the launcher execs (`BUILD_BIN`=codegen-build, resolved via `OCG_CODEGEN_DIR`/`SCRIPT_DIR`, cwd-independent) — NOT the agent binary. Stub: temp dir, binary named `codegen-build`, `OCG_CODEGEN_DIR=stub_dir`; captures invocation args, exits 0. Assertions verify correct args (e.g. harness-appropriate pitch mention: `@codegen/pitches/ready/<slug>.md`) before exec. Isolates launcher path logic, hermetic, no `@moduletag :slow`.
...
- **Fixture prompt suffixes must not contradict the loop's own commit step** — appending `@commit_contract_suffix` or similar fixture-embedded "commit directly via bash" instructions conflicts with the loop's deterministic `codegen-commit` step (no agent commits — see `pre-commit-guard.sh`). If obeyed → the commit attempt is denied and `loop_failed` on the resulting error; if ignored → reviewer correctly reds for contradicting discipline. The loop's `assert_work_produced!` is the strongest guard. Always audit build prompts for role conflicts.
