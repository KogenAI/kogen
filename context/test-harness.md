...
- **Telemetry** — `accumulate_telemetry/3` (`/2` delegates) sums `usage` (cost/tokens/turns) into a process-dict accumulator per role incl. retries; 3rd arg = requested `{harness,model,effort}` dispatch, read via `UsageParser.parse_dispatches/1`. `Mix.Tasks.Codegen.Loop.emit_loop_telemetry/1` prints one aggregated line after `run/1`. Campaign binding → `context/loop.md`.
...
- **Cycle-level resume** (`resume_checkpoint/3`) — turn-0 check for durable checkpoint (state ∈ {GATED, REVIEWED, CURATED} + gate clear + tree match + publishable working-tree changes + HEAD unmoved). Valid → resume at mapped role, skipping expensive prefix; clean/stale/invalid → `:full`. Interrupted recovery converts persisted `resume_pending` + `:full` into checkpoint/journal retirement and ordinary requeue without moving HEAD. Seams (also forwarded by queue drain): `:cycle_state_get_fn`, `:read_verdict_fn`, `:gate_tree_match_fn`, `:resume_work_present_fn`, `:gate_result_base_sha_fn`, `:log_resume_fn`. Tests: "resume-checkpoint", `interrupted_cycle_recovery_test.exs`, drain startup recovery.
