...
| `test_harness/lib/codegen_test_harness/loop_queue.ex`         | Kahn topo-sort over `Blocks-on:`/`## Dependencies` edges + transient-error classification (mirrors `harnesses/shared/retryable-errors.sh`). The former bash queue script this ported from (harnesses/shared/build-queue.sh, removed) is deleted — process-orchestration concerns (lock, watchdog, per-pitch budget) were intentionally NOT ported (the loop drives one pitch per invocation). Hermetic tests in `loop_queue_test.exs`. |
...
## Orchestration Loop (`OrchestrationLoop`, `LoopGate`, `LoopQueue`, `mix codegen.loop`)
...
- Test seams: `:invoke_fn`, `:gate_fn`, and (on `invoke_role/4`) `:resolve_fn`/`:codegen_call_fn` opts let `orchestration_loop_test.exs` stub the LLM/gate entirely — 13 hermetic cases, no `:slow` tag, runs under `make test`.
- `LoopGate.run_gate/2` shells `gate-select.sh`'s `gate_select_decide` for the gate command, executes it, then `gate-result.sh`'s `write_gate_result` for the verdict — reuses the same `codegen/gate-pending/` gate-result JSON schema byte-identical to the pre-loop schema (the `codegen-build` post-step section "Post-step: Pi must present a structured clear gate result; Claude may synthesize one" still applies unchanged).
- `LoopQueue` mirrors `retryable_regex` transient classification (source of truth: `harnesses/shared/retryable-errors.sh`) and Kahn topo-sort over `Blocks-on:`/`## Dependencies` edges. Process-orchestration concerns (queue lock, watchdog, per-pitch wall-clock budget, a persisted queue-position file) were never ported — the loop drives one pitch per invocation; multi-pitch draining is the caller's concern.
- State advancement (GATED→REVIEWED→CURATED→COMMITTED) shells `cycle-state.sh`'s `write_cycle_state` via `OrchestrationLoop.advance_cycle_state_step/3` — single source of truth, no re-implemented state store.
- **Cutover complete**: the in-harness self-orchestration surface this loop replaces (A-bucket SubagentStop/Stop role-sequencing hooks, the bash queue script (was harnesses/shared/build-queue.sh), the orchestrator rule files (was roles/orchestrator.md + stacks/phoenix/orchestrator.md), `_orch-*.md.j2` fragments, `curator-format.sh`/`post-developer-format.sh`/`stop-verify-planner-gate.sh`) is DELETED — not coexisting, not flag-gated. The loop covers their function as explicit steps (`run_format_step/2`, crash-loud gate-parse-error handling). The loop is the sole non-interactive build driver. The two build-mode prompt-body files are RETAINED (reworded to drop dead-hook references) — they remain the system prompt for the surviving interactive/resumable-session fallback in `dispatch.sh` (see `context/subagent-influence-stack.md`), which still self-orchestrates.
...
- **Multi-module ExUnit files** — private helpers cannot be shared across modules in same file; promote to public in support module (e.g. `Fixtures.count_commits!/1`) or keep private per-module copy
- **phx_new flags** — version 1.8.7+ does not support `--force` flag; scaffold via plain `mix phx.new . --app <name> --live`; any direct scaffold call must mirror this exact incantation for consistency with fixture setup
