...
- `guard_bundle_flag!("claude_code")` defaults to the committed minimal loop bundle `harnesses/claude/claude-code-loop-settings.json` (8 role-agnostic hooks), not the full settings.json — loop calls run without role identity. See `context/core.md` § Loop Settings Bundle (`LOOP_BUNDLE_IDS`). Legacy path unaffected (no caller of `guard_bundle_flag!/2`).
- Test seams: `:invoke_fn`, `:gate_fn`, and (on `invoke_role/4`) `:resolve_fn`/`:codegen_call_fn` opts let tests stub the LLM/gate entirely. Hermetic test pattern: use `Keyword.merge(defaults, extra)` NOT `list1 ++ list2` when merging `_fn` opts (left operand wins in concat, right wins in merge — applies to all seam-based test helpers like `orchestration_loop_test.exs` and `loop_queue_drain_test.exs`).
