...
- **Turn-0 orientation-doc preflight** (`preflight_orientation_docs!/2`) — after `preflight_roles!/3`. Shells index-parity (repo-wide) + factcheck whole-tree (codegen-repo sentinel only). Inherited violation → `InfraAbort` (exit 3, pitch stays `ready/`), not a paid cycle dying at `run_curator_doc_check`. Tests: describe "turn-0 orientation-doc preflight".
