...
| `test_harness/lib/mix/tasks/codegen.pitches.scope.ex`         | `mix codegen.pitches.scope [--dir=ready\|draft\|shipped] [--cwd=<dir>] [--lanes=N]`. No `--lanes`: byte-identical report (`scope_report/1`). `--lanes=N`: adds LANE 1..N + GLOBAL-HOT (`partition/2`). Print-only. Tests: `codegen_pitches_scope_test.exs`. |
...
- **Owner-routing/flake/marker seams** — `:gate_classify_fn` is 2-arity (`(cwd, dev_role) -> {:owner, role} | :infra`). `:flake_check_fn` (default: `:gate_fn`) backs the one flake re-run before rework. Drain's `:terminal_marker_fn` reads `terminal-state.json` — see loop/loop-queue-drain owner files.
