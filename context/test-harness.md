...
- `LoopQueue` mirrors `retryable_regex` transient and Kahn topo-sort. `LoopQueueDrain.drain/1` live caller: `--elixir --queue` execs `mix codegen.loop.queue`; bare `--queue` execs legacy `harnesses/shared/build-queue.sh`. Both share `codegen/gate-pending/` lock. **Operator output (new)**: per-pitch banner `[N/total] ... building`, two-path echo (session.md + jsonl, fail-open), streamed child stderr to operator, terminal outcomes. Session.md discovery via glob with ≤30 polls; tee via `IO.binwrite(:stderr, chunk)`. Tests: `loop_queue_drain_test.exs`.
- **Committed-but-nonzero recovery** — `handle_nonzero_exit/5` detects committer-post-commit hiccup (HEAD moved + gate `"clear"`) and counts pitch shipped instead of erroring. Seams `:git_head_fn`, `:gate_verdict_fn` with fail-open defaults. Only `"clear"` recovers; `"failed"`/`"inconclusive"`/`""` halt loud. Tests: `loop_queue_drain_test.exs` (`6r1`-`6r6`).
  - **Fixture discipline**: spawn only for pitches in `ready_dir`; recovery scenario must seed `ready_dir` + have spawn_fn perform ready→shipped rename before non-zero return.
  - **Call-count notes**: `committed?` short-circuits `git_head_fn` to 1 call when `head_before` nil/empty.
  - **Idempotency contract**: `LoopQueueDrain.ship/3` MUST be idempotent (return `:ok` if dst already exists in shipped/) because the build agent already moves ready→shipped itself per the baked claude-build-system-prompt.txt contract. Drain checks dst presence before assuming failure; never fights agent for ownership of the move. Tests 6r1/6r2 already exercise agent-pre-ships paths. Exit-0 path now covered directly via ship/3 idempotency tests.
