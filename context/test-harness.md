...
- **Warm-resume on transient retry** (`do_invoke_attempt/6`, `invoke_role/4`, `resume_prompt/1`) — transient failure resumes the SAME session (mint via `mint_session_id/0`, `--session-id` cold / `--resume` retry, short continuation prompt) instead of re-invoking cold. `stale_session_reason?/1` falls back cold on an unresumable session. `CODEGEN_RESUME_ATTEMPT=<sid>` lets `developer-no-self-gate` treat a new token as fresh-run not spin.
...
- `LoopQueue` mirrors `retryable_regex` transient and Kahn topo-sort. `LoopQueueDrain.drain/1` is the sole `--queue` drainer: `claude-build.sh`/`pi-build.sh --queue` always execs `mix codegen.loop.queue`. Shares `codegen/gate-pending/` lock with single-pitch builds. **Operator output**: per-pitch banner, two-path echo (session-log `_cycle.jsonl` primary + `_build.log` console capture secondary, fail-open), streamed child stderr, terminal outcomes; session-log discovery via glob ≤30 polls. Best-effort prune of `_build.log` files older than 7 days at drain leg-1 (never `_cycle.jsonl`). Tests: `loop_queue_drain_test.exs`. **Per-pitch child**: `default_spawn_fn/5` invokes `codegen-build --harness=<h> --stack=<s> --cwd=<c> -- <pitch>` — no engine flag.
- **Committed-but-nonzero recovery** — `handle_nonzero_exit/7` detects committer-post-commit hiccup (HEAD moved + gate `"clear"` + fresh) and counts pitch shipped instead of erroring. `committed?` is forward-only (`head_before` must still be an ancestor of new HEAD, via `:git_ancestor_fn`); a non-descendant move (orphaned base) halts loud with a `git rebase --onto` remediation, pitch left in `ready/`. Tests: `loop_queue_drain_test.exs` (`6r1`-`6r6`, `6r-orphan`, `6r-forward`, `6r-fresh-*`).
- **Gate-record freshness** — `LoopGate` writes `base_sha` = short HEAD **at gate time** (`""` if non-git) — gate runs BEFORE the committer, so this sha only ever prefixes `head_before`, never post-commit `head_after`. `gate_fresh?/3` (shared by exit-0 + nonzero-hiccup paths) also requires gate mtime (`:gate_mtime_fn`) `>=` spawn `ts`, rejecting a stale verdict from an earlier non-committing cycle sharing the same base. Tests: `loop_gate_test.exs` real-git-repo reconciliation.
- **Gate content binding** — `base_sha` pins HEAD only, not content. `LoopGate` also stamps `graded_tree_sha` (git tree object, HEAD + working-tree changes via a temp index; needs `git update-index -q --refresh` first, else collapses to `HEAD^{tree}`). `ensure_gate_graded_this_tree!/5` re-compares stamped vs. current tree just before the committer runs; mismatch re-gates (bounded `:max_final_gate_cycles`, default 1) via `do_gate_loop/9`'s rework shape. `assert_commit_matches_gate!/1` re-checks post-commit. `""` either side → skip. Tests: `loop_gate_test.exs`/`orchestration_loop_test.exs` "graded_tree_sha"/"no ship on a gate that didn't grade this tree".
...
- **Isolated deterministic failure — skip-and-continue, not a halt** — deterministic failure (or retry exhaustion) SKIPS-AND-CONTINUES: pitch stays `ready/`, dirty tree committed to `queue-fail/<slug>/<ts>` branch (`:git_stash_fn` "fail": stash push -u + branch-cut + drop, fail-open — never an invisible stash, per `no-git-stash.sh`), named + `git checkout` recovery in FAILED bucket/HALTED msg, NEVER auto-restored (`:git_stash_restore_fn` matches only `queue-timeout:`). `"timeout"` unchanged (stash push, popped before retry). `drain/1` still `{:ok, shipped_count}`. Circuit breaker `:max_consecutive_fails` (default 3, env `CODEGEN_BUILD_QUEUE_MAX_CONSECUTIVE_FAILS`) HALTs on threshold. Tests: `loop_queue_drain_test.exs` (`1h`,`5`,`6`,`6r4`,`6r5`, circuit-breaker + branch-park cases).
...
- **`build_prompt/2` testability & reviewer file set** — Prompt-assembly point in `OrchestrationLoop.build_prompt(role, ctx)`. `def`+`@doc false` for direct ExUnit calls. Reviewer branch renders loop-derived `## Files Modified` list (`invoke_reviewer/4` → `default_review_file_set_fn/1`, mirrors `default_rework_brief_fn/1`'s git idiom); content via `git diff HEAD -- <path>`. Empty set in real git tree → loop refuses reviewer invoke (fail-loud). Gate-clear drops `:last_failure_reason` — no stale leak into reviewer prompt.
