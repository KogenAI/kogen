...
- `OrchestrationLoop.run/1` — sequences roles via `invoke_role/4` (real `RoleResolver.resolve_role/2` → `codegen-call`), interleaves `LoopGate.run_gate/2` after the developer role. `codegen-call` envelope `result.status`: `"success"` advances, `"failed"`/`"clarifying_question"` retries the SAME role once (folding reason into retry prompt) then `{:error, reason}`; any other shape RAISES (crash loud, never silently continues). Gate `:failed`/`:inconclusive` re-runs developer role up to `:max_gate_retries` (default 1) before giving up.
