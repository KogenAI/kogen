...
| `test_harness/lib/codegen_test_harness/loop_queue.ex`         | Kahn topo-sort + transient-error classification; ported from removed `build-queue.sh`. Parses `scope:` (`parse_scope/2`, `scope_report/1`); folds into lanes (`partition/2`). See "Orchestration Loop" § below. Tests: `loop_queue_test.exs`. |
...
`codegen-build` has no engine flag; `dispatch.sh` always execs `mix codegen.loop`. Shared ExUnit helpers (`run_codegen_build/3`, `run_codegen_build_parity/4` in `fixtures.ex`) call `codegen-build` with `--harness`/`--stack`/`--cwd` only.
...
- **Reviewer→dev fix cycle** — reviewer ends with `REVIEW_VERDICT: APPROVED|CHANGES_REQUESTED`. `CHANGES_REQUESTED` re-invokes dev, re-formats, re-gates, re-reviews (within `:max_review_cycles`, default 1).
...
- **Telemetry** — `accumulate_telemetry/2` sums each envelope's `usage` (cost/tokens/turns) into a process-dict accumulator (`get_telemetry/0`/`zero_telemetry/0`) per role, incl. retries. `Mix.Tasks.Codegen.Loop.emit_loop_telemetry/1` prints one aggregated `{"type":"result",...}` JSON line after `run/1` regardless of outcome, for the benchmark harness.
- **Gate selection** (`LoopGate.decide_gate/1`, `gate-select.sh`) — 2-source tree: (1) planner Tier-1 `**Gate**:` block (JSONL parsed), wins over (2) per-app `.claude/gate-config.sh` `GATE_COMMAND=<cmd>`. Falls back fail-loud `__GATE_UNRESOLVED__` (raises). Static tests MUST supply GATE_COMMAND or planner step_log; no stack-default fallback.
...
- Test seams: `:invoke_fn`, `:gate_fn`, `:gate_preflight_fn`, `:advance_cycle_state_fn`, and (on `invoke_role/4`) `:resolve_fn`/`:codegen_call_fn` opts let tests stub the LLM/gate/state-write entirely. Use `Keyword.merge(defaults, extra)` NOT `list1 ++ list2` (left wins in concat, right wins in merge). **Race convention**: any `async: true` test sharing literal `cwd: "/tmp/irrelevant"` that can reach `:clear` gate MUST stub `:advance_cycle_state_fn` to a no-op (prevents concurrent `mkdir -p "$cwd/codegen/gate-pending"` races) — or use a per-test unique tmpdir. See `orchestration_loop_test.exs` `no_op_advance_cycle_state_fn/0`.
- **Owner-routing/flake/marker seams** — `:gate_classify_fn` is 2-arity (`(cwd, dev_role) -> {:owner, role} | :infra | :stale_build`). `:flake_check_fn` (default: `:gate_fn`) backs the one flake re-run before rework. `:stale_build_heal_fn` (default: nukes `_build`) backs the `:stale_build` leg — one rebuild + gate re-run (attempt not spent), else falls through to owner rework (`do_gate_loop_stale_build_heal/10`). Drain's `:terminal_marker_fn` reads `terminal-state.json` — see loop/loop-queue-drain owner files.
...
- **Single-pitch move** — `Mix.Tasks.Codegen.Loop.run/1` is the primary shipper in BOTH modes (`--queue` spawns a solo `mix codegen.loop` child on a `ready/` pitch). `verify_commit_landed/2` returns `{:ok, after_sha | nil}` (was bare `:ok`); `maybe_ship_pitch/4` (`source, cwd, before_sha, after_sha`) moves `ready/<slug>.md` → `shipped/<slug>.md`, calling `LoopQueue.record_ship/4` first — writes ONE thing: `shipped_sha:`/`shipped_range:` frontmatter, stamped before the mv (a died-before-mv failure never strands an already-shipped stamp on a pitch still sitting in `ready/`, which the next build's `@`-mention would misread). There is no second medium (no git note) — the frontmatter stamp in the pitch file is the sole ship record. Fails open on non-git/nil sha. `LoopQueueDrain.ship/3` (now 6-arity) is the FALLBACK. Tests: `codegen_loop_test.exs`, `loop_queue_test.exs` describe "record_ship/4".
- `LoopQueue` mirrors `retryable_regex` transient and Kahn topo-sort. `LoopQueueDrain.drain/1` is the sole `--queue` drainer: `claude-build.sh`/`pi-build.sh --queue` always execs `mix codegen.loop.queue`. Shares `codegen/gate-pending/` lock with single-pitch builds. Operator output: per-pitch banner, two-path echo (`_cycle.jsonl`+`_build.log`, fail-open), streamed stderr, terminal outcomes; `_build.log` gzipped at 14 days, deleted at 30 days. Tests: `loop_queue_drain_test.exs`. Per-pitch child: `default_spawn_fn/5` invokes `codegen-build --harness=<h> --stack=<s> --cwd=<c> -- <pitch>`.
...
- **Publish-on-ship** — `:publish_preflight_fn` runs once before any spawn (fail-closed). Each ship site calls `publish_or_halt/4` → `:git_publish_fn`: ff/push, rebase+push (re-stamps `record_ship/4`), or conflict → recovery branch + HALT. Never `--force`. Full contract → `context/loop-queue-drain.md`.
