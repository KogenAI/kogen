...
- **Isolated deterministic failure — skip-and-continue, not a halt** — a deterministic child failure (or a pitch that burns its transient retries) SKIPS-AND-CONTINUES: pitch stays in `ready/`, dirty tree stashed under `queue-fail:<slug>:<ts>` (fail-open, arity-3 `:git_stash_fn`, never auto-popped), slug joins `failed_slugs` (rejected on later scans), `drain/1` still returns `{:ok, shipped_count}` with a FAILED bucket stderr line. Guarded by a consecutive-failure circuit breaker (`:max_consecutive_fails`, default 3, env `CODEGEN_BUILD_QUEUE_MAX_CONSECUTIVE_FAILS`) — any ship resets the streak; hitting threshold HALTs `{:error, reason}`. Orphaned-base/lock contention unaffected (still immediate halts). Tests: `loop_queue_drain_test.exs` (`1h`, `5`, `6`, `6r4`, `6r5`, circuit-breaker + stash-reason cases).
