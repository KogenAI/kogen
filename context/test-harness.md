...
- `LoopQueue` mirrors `retryable_regex` transient classification and Kahn topo-sort. `LoopQueueDrain.drain/1` is its live caller, gated like the one-shot build: `--elixir --queue` execs `mix codegen.loop.queue`. Bare `--queue` (default) execs legacy `harnesses/shared/build-queue.sh` directly (not via dispatch.sh's exec-env), reading its own per-pitch budget toggles from ambient env. Elixir path re-scans `ready/` (raises on cycle), spawns fresh `codegen-build` per pitch (budget `CODEGEN_BUILD_QUEUE_PITCH_BUDGET_SECS`), timeout→stash, lock at `queue.lock`. Tests in `loop_queue_drain_test.exs`.
