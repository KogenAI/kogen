...
- `OrchestrationLoop.run/1` — sequences roles via `invoke_role/4` (real `RoleResolver.resolve_role/2` → `codegen-call`), interleaves `LoopGate.run_gate/2` after the developer role. `codegen-call` envelope `result.status`: `"success"` advances, `"failed"`/`"clarifying_question"` retries the SAME role once (folding reason into retry prompt) then `{:error, reason}`; any other shape RAISES (crash loud, never silently continues). Gate verdict is BINARY (`:clear | :failed`) — `read_verdict/1` collapses legacy `"inconclusive"` fail-closed to `:failed`; `do_gate_loop` RAISES on any stray verdict (defensive). For `stack: "static"`, `run_gate/2` runs `:preflight_fn` (default `static_render_deps_preflight!/1`) BEFORE the gate command — RAISES (infra abort, not a verdict) naming the first missing render-check dep (node, render-check.js, chromium), mirroring render-check.js's playwright/chromium resolution. Phoenix skips preflight. Loop-only — legacy bash gate hooks keep ternary verdict unchanged.
...
- `LoopQueue` mirrors `retryable_regex` transient and Kahn topo-sort. `LoopQueueDrain.drain/1` is its live caller: `--elixir --queue` execs `mix codegen.loop.queue`; bare `--queue` execs legacy `harnesses/shared/build-queue.sh`. Both engines share `codegen/gate-pending/` (lock at `queue.lock`, build-queue.json manifests) — Elixir removes stale legacy manifests at startup and uses same lock path, so live locks are honored by both engines (preventing concurrent runs).
- **Committed-but-nonzero recovery (ported from legacy `build-queue.sh`)** — `LoopQueueDrain.handle_nonzero_exit/5` no longer halts the whole queue on every non-zero child exit. New seams `:git_head_fn` (default reads `git rev-parse HEAD`, fail-open `nil`) and `:gate_verdict_fn` (default reads `codegen/gate-pending/gate-result.json` `.verdict` via Jason, fail-open `""`) let it detect a committer-post-commit hiccup: HEAD moved during the spawn AND gate verdict is `"clear"`. When both hold, the pitch is counted shipped (finishing the `ready/` → `shipped/` move itself if the agent's own ship step didn't run, or just counting it if the file is already in `shipped/`) instead of erroring. Retry-on-nonzero is now `transient?/1 OR (gate-clear AND not-committed)`, still bounded by `max_retries`. Only verdict `"clear"` recovers — `"failed"`/`"inconclusive"`/`""` always halt loud. Tests: `loop_queue_drain_test.exs` (`6r1`-`6r6`).
  - **Test fixture discipline**: `run_slug/2` only spawns for pitches present in `ready_dir` (via `LoopQueue.ordered_slugs/1`); an "already shipped" recovery scenario must seed the pitch file in `ready_dir` and have `spawn_fn` itself perform the ready→shipped rename (simulating the agent's own committer) before returning non-zero. Seeding only `shipped_dir` never triggers a spawn.
  - **Call-count assertions**: `committed?` short-circuits `git_head_fn` to a single call when `head_before` is `nil` or `""` (never invokes a second time for the post-spawn comparison). Tests tracing exact call counts must account for this short-circuit; do not assume unconditional 2 calls per attempt.
