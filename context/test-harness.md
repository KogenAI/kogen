...
- **Turn-0 role-agent resolution preflight** (`preflight_roles!/3`) — `run/1` confirms every role in the resolved sequence resolves as an installed `--agent` BEFORE any role is invoked, via `:preflight_probe_fn` (default `default_preflight_probe/1`: sentinel `codegen-call --agent __codegen_loop_preflight_probe__` — claude exits 1 immediately with `Available agents: <csv>`, zero model turns). `missing = roles -- available` non-empty → raises naming missing roles + available list + `make install` hint. No parseable line → raises (fail-closed). Prevents a broken role set surfacing mid-cycle as a mis-labeled transient retry. Tests: `orchestration_loop_test.exs` describe "turn-0 role-agent resolution preflight".
...
- `LoopQueue` mirrors `retryable_regex` transient and Kahn topo-sort. `LoopQueueDrain.drain/1` is the sole `--queue` drainer: `claude-build.sh`/`pi-build.sh --queue` always execs `mix codegen.loop.queue`. Shares `codegen/gate-pending/` lock with single-pitch builds. **Operator output**: per-pitch banner, two-path echo (`_cycle.jsonl` primary + `_build.log` secondary, fail-open), streamed child stderr, terminal outcomes; log discovery via glob ≤30 polls. Best-effort prune of `_build.log` files older than 7 days at drain leg-1. Tests: `loop_queue_drain_test.exs`. **Per-pitch child**: `default_spawn_fn/5` invokes `codegen-build --harness=<h> --stack=<s> --cwd=<c> -- <pitch>` — no engine flag.
- **Committed-but-nonzero recovery** — `handle_nonzero_exit/7` detects committer-post-commit hiccup (HEAD moved + gate `"clear"` + fresh), counts pitch shipped instead of erroring. `committed?` forward-only (`head_before` must be an ancestor of new HEAD, via `:git_ancestor_fn`); non-descendant move halts loud with `git rebase --onto` remediation, pitch left `ready/`. Tests: `loop_queue_drain_test.exs` (`6r1`-`6r6`, `6r-orphan`, `6r-forward`, `6r-fresh-*`).
  - **Idempotency contract**: `LoopQueueDrain.ship/3` MUST be idempotent (return `:ok` if dst exists) — the build agent already moves ready→shipped per baked contract; drain never fights it for ownership.
...
- **Turn-0 orientation-doc preflight** (`preflight_orientation_docs!/2`) — after `preflight_roles!/3`. Shells index-parity (repo-wide) + factcheck whole-tree (codegen sentinel only). Inherited violation → `InfraAbort` (exit 3, pitch stays `ready/`), not a paid cycle dying at `run_curator_doc_check`. Tests: describe "turn-0 orientation-doc preflight".
