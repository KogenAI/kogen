...
A test fixture can encode the SAME false premise as the production bug it is meant to catch — keeping suite green through multiple commits of regression. Example (pitch: queue-drain-gate-freshness-inverts-commit-order): drain false-FAILED bug required `base_sha` prefix of post-commit HEAD (`head_after`), unsatisfiable since gate runs pre-commit. The suite's `shipped_opts/2` mock returned **latest HEAD** (= `head_after`), exactly the bug's false premise. Suite stayed green because both modeled the same wrong world. **Fix**: re-model fixture to correct assumption (sha = `head_before`, pre-commit). Add producer/consumer reconciliation tests running REAL producer (e.g., `LoopGate.run_gate/2` in real tmp git repo) and asserting REAL consumer readers satisfy consumer's own freshness predicate — that test class was absent, letting the false premise hide.
