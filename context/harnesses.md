...
**Shaper ask-vs-decide classifier (I)**: Ask only when answer changes product behavior. Never-ask: tools, models, env, naming, placement, probe depth/thoroughness/cost, and fail-closed-vs-fail-open whether or not the requirement CAN be guaranteed (the un-guaranteeable case routes to the operator-surface disposition `BREAKING`, never a question). Auto-decide engineering choices; record `Assumed: key=value`. Sibling of `shaper-discipline.md` § Ask-vs-Decide Classifier.
...
**Decompose-then-split rule** (Rule G): SPLIT multi-surface problems into independent pitches (eng-decomposition, not user choice). Only product forks reach user. Before writing, rule G runs the one-clause test (rule H outcome (e)) itself: one-purpose pieces collapse into one pitch, never split; a surviving split records `split_subject: A; B` per sibling — `mix codegen.pitches.scope --check` fails an unproven SUBSUMED pair. **Split tripwire**: additionally, if a sibling about to be created would declare `blocks_on: [source-pitch]`, run the standalone-promise test BEFORE the file exists — does the source pitch keep its OWN stated promise if it ships alone? No → fold the sibling back, no new file. This fires at creation time even when the sibling has no `scope:` yet (so the mechanical SUBSUMED check can't see it).
