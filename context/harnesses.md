...
**Pi bounded call transport**: managed Pi output passes through tracked `pi-jsonl-filter.cjs` before temp/durable capture; only parsed top-level `message_update` snapshots are omitted. The dispatcher runs from an immutable temp snapshot, owns Pi through a process-group supervisor, records the actual Pi PID for tool-child detection, and treats Pi+filter as one lifecycle. Terminal `agent_end` remains salvageable; every filter/setup fatal path terminates and reaps Pi before failing. Repeated `codegen-call --extension` values retain caller order and become repeated Pi `--extension` argv entries.
