...
**Committer spawn**: the deterministic `OrchestrationLoop` spawns the committer role directly as the loop's COMMITTED step (see `context/test-harness.md`) for non-interactive builds. The interactive/resumable-session fallback still spawns it via the `AGENTS-{phoenix,static}.md.j2` session-loop prose (the former shared `_orch-committer.md.j2` include fragment was retired this cutover).
...
- **`dev-gate` is not a subagent template** — `dev-gate` refers to the gate step of the build cycle (owned by the Elixir loop's `LoopGate` for non-interactive builds, or by a SubagentStop hook for the interactive-session fallback), not an agent role with a `.md.j2` template; see `context/hooks.md` for gate mechanics
