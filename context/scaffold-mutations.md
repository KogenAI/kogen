...
- **Scaffold.sh Phase pattern for new directories**: `mkdir -p` + echo status message. NEVER a sentinel file (`.gitkeep`/`.keep`) — `priv/plts/` and `codegen/pitches/{draft,ready,shipped}/` both sit under unnegated `.gitignore` boundaries, so a sentinel there cannot survive a clone; it only looks solved. `run_integrate_stage` (`codegen-scaffold`) is the durable recreator, run on every `create`/`integrate`; the scaffold-time `mkdir -p` only covers first provision (see `context/scaffold.md` § clone survivability).
