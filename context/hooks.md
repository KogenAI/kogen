...
**`committer-bash-allowlist` pattern (generated from registry)**: Regex in `shared/enforcement/registry.yaml` defines Bash commands committer may invoke: optional `(cd\s+\S+\s+&&\s+)?` prefix + allowlisted commands (git subcommands optionally preceded by `-C <path>`, or `echo`/`wc`/`ls`/`true`). Supports both `cd /path && git commit` and `git -C /path commit`, denies non-allowlisted commands after a `cd` prefix (no shell-escape loophole). Generated (read-only); source is `registry.yaml`. See `shared/rules/_core/bash-discipline.md` § Renderer-Neutral Regex Tokens.
...
| `harnesses/claude/hooks/portable-launcher_test.sh` | Real launcher tests, all 4 callers (`claude-shape/experiment`, `pi-shape/experiment`): CONTEXT_FLAGS/ROLE_SYSTEM_PROMPT append, drift-block, pitch resolution, Tier-0/Tier-1 loading via `pitch-context-selector.sh` (fail-open, citation priority, 6-row cap, missing-file error) |
| `harnesses/claude/hooks/build-launcher-wrapper_test.sh` | Hermetic wrapper tests for claude-build.sh + pi-build.sh: CODEGEN_DIR 3-branch resolution, --queue dispatch, basename resolution, cwd normalization. ALL terminal execs stubbed. 13×2 launchers = 114 assertions. Sibling: `loop-signal-bridge_test.sh` covers the queue signal-teardown path. |
...
| `harnesses/claude/hooks/run-tests.sh` | Auto-discovers/runs all `*_test.sh` via `find "$HOOKS_DIR" -name '*_test.sh'`; new tests need ZERO Makefile wiring (exempt from HOOK-MANIFEST via `--exclude-pattern=_test.sh`). Under `make test`, `run-all-tests.sh` passes `HOOK_TEST_EXCLUDE` (paths already owned by a direct target e.g. `prompt-content-parity`) so each test has exactly ONE owner; unset/standalone still runs full population. |
...
**Hermetic CI tests vs hooks**: `context-doc-provenance_test.sh` scans docs/rules for rotting line-citations — not a hook/registry entry, absent from `settings.json`. Auto-discovered via `run-tests.sh` footer (`N passed, N failed`). Full-tree context-index parity is NOT a `make test` gate — see `context/loop.md` § Curator-Doc Check for the turn-0 preflight + in-loop scan mechanism (both route to context-curator, never the developer). Replaces deleted `context-index-coverage_test.sh`.
