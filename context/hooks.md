...
**Not-yet-migrated hooks:** migration is complete — there are no current outliers. `subagent-read-discipline.sh` is a live `kind: registration` entry in `registry.yaml`, not a deferred/hand-authored header. Prior `role: unset` outliers (`context-index-parity.sh`, `context-file-size-gate.sh`, `context-factcheck-guard.sh`) were deleted — moved to `context-factcheck-edit-gate.sh` + in-loop `run_curator_doc_check`. `registry.yaml`'s header comment (§ NOT-YET-MIGRATED) is the single authoritative note on this state — see it, don't re-narrate it here.
...
**codegen-log write bypass (shared helper)**: 13 command-scanning PreToolUse Bash guards (developer/dev-no-ci/committer-\*/orchestrator-no-ci/no-git-stash/no-cat-pipe/developer-static-no-manual-build) each call a shared `is_codegen_log_write()` helper (`hooks-lib.sh`) / `isCodegenLogWrite()` (Pi `hook-helpers.ts`) BEFORE any phrase grep or counter increment — a `codegen-log` invocation is always a session-log WRITE, never the gated action its heredoc body narrates. Mirrors the bypass in `session-log-writer-only.sh`/`.ts` and `pre-commit-guard.sh`/`planner-guard.sh`.
...
| `SubagentStop` | developer-no-self-gate-reset | Non-build-modes-only: gate verdicts. Dead under the loop (no SubagentStop per-role) — loop covers gate exec (`LoopGate`), formatting (`run_format_step`), cycle-state advancement, env-var sample-consistency (`run_env_var_step` → `harnesses/claude/hooks/lib/env-var-sample-scan.sh`, diffs vs HEAD, flags ANY newly-added string-literal env-var read (bare/defaulted/`||`/`case`/`==`/`fetch_env!`) undeclared in BOTH sample files — documentation axis, not crash-ability; ambient OS/shell vars (`HOME`,`PATH`,`LC_*`) exempt via allowlist; relocated off committer), curator factcheck + index-parity (`run_curator_doc_check` → `context-factcheck-scan.sh` + `context-index-parity-scan.sh`; 3 claim classes: named-path, count-anchor, id-corruption). Pre-loop equivalents deleted (covered above). |
...
`harnesses/claude/hooks/context-factcheck-edit-gate.sh` — PreToolUse: denies a write to any orientation doc whose PROJECTED post-write content fails `context-factcheck-scan.sh` (temp mirror). Writer's own turn. **Doc-root split**: `FACTCHECK_DOC_ROOT` reads doc from mirror; path/count probes resolve against real `repo_root` — else every valid claim falsely denies. Both twins set both. **Named-path escapes** (two categories): (1) downstream-app paths need a placeholder segment `` `<app>/context/core.md` `` (out of grammar by `<`); (2) gitignored machine-local paths (e.g. `codegen/drain-nodes.yaml`) need nothing — claim-miss checked via `git check-ignore -q`, skipped on rc 0 (ignored), violated otherwise. Index-aware: tracked paths never report as ignored, even if force-added past a rule and later deleted.
...
- `append <role> --plan @- --slug <slug>` — typed PLAN document (raw text, not JSON; blank exits 2). planner* authors it; `gate-select.sh`'s `gate_select_read_planner_plan` → `LoopGate.planner_plan/1` → `resolve_planner_plan!/2` thread it, never the role body. Absent/blank raises before a developer runs.
