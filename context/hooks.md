...
| `harnesses/claude/hooks/planner-guard.sh`                       | PreToolUse — enforces planner constraints (no writes, no bash exec); blocks Bash redirects to `codegen/logging/*.md` including shell heredocs and `>>` appends (prevents transcript breakage from Bash tool_use entries that miss JSONL parsing gates); **implication**: session log version-stamps must be written via Edit tool, NOT bash heredoc patterns (`cat >> file <<EOF`) |
...
**prompt-content-parity test scope**: The `prompt-content-parity_test.sh` hook asserts **only the shape mode sentinels** (`ASK-GATE: product forks only`, `INTERACTION-AUDIT`) in baked `claude-shape-system-prompt.txt`. Edits to `ops.txt` / `debug.txt` / other prompt-body files that introduce **no sentinel** cannot break this test — useful to confirm before assuming a prompt-body edit needs a parity-test update. If a prompt-body edit introduces new sentinel text (e.g., "ASK-GATE: another constraint"), the test sentinel in `harnesses/claude/hooks/prompt-content-parity_test.sh` must be updated to assert it.
