...
Replaced by **`role-retrospective-before-stop`**, a `Stop`-event hook gating `developer-*|reviewer-*` (curator/committer exempt). Claude twin BLOCKS: pushes the role back into its OWN warm session naming the exact `codegen-log` command. Pi twin is observe-only (`session_shutdown` cannot block). Checks per role: non-empty `ev:role` body + EITHER `ev:learned` OR `ev:no_learning`. Substance enforced at the writer — `codegen-log` refuses placeholder/compliance-echo payloads (exit 2). Bounded at 3 blocks/session, then falls through — never fails the build. `section <role> --learned "<text>"` is the one-call path; `append <role> --no-learning "<text>"` is the legal empty-turn exit (session-log rules § Ownership).
