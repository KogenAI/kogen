...
**codegen-log write bypass (shared helper)**: 13 command-scanning PreToolUse Bash guards (`developer-no-self-gate.sh`, `dev-no-ci.sh`, `committer-single-commit-per-cycle.sh`, `committer-no-revert-prior-commit.sh`, `committer-no-head-move-reset.sh`, `committer-no-trailer-guard.sh`, `committer-single-line-guard.sh`, `committer-subject-length.sh`, `env-var-sample-consistency.sh`, `orchestrator-no-ci.sh`, `no-git-stash.sh`, `no-cat-pipe.sh`, `developer-static-no-manual-build.sh`) each call a shared `is_codegen_log_write()` helper (`hooks-lib.sh`) / `isCodegenLogWrite()` (Pi `hook-helpers.ts`) BEFORE any phrase grep or counter increment. A `codegen-log` invocation is always a session-log WRITE, never the gated action its heredoc body narrates (e.g. a log entry saying "ran make ci" must not itself be denied/counted as `make ci`). Mirrors the bypass already present in `session-log-writer-only.sh`/`.ts` and `pre-commit-guard.sh`/`planner-guard.sh`.
...
| `harnesses/claude/hooks/env-var-sample-consistency.sh` | SubagentStop (developer-phoenix-backend\|developer-phoenix-frontend) — blocks (re-spawns) when the working-tree diff vs HEAD adds env-var-reading code without updating .env.sample/.env.prod.sample |
...
| `SubagentStop` | developer-no-self-gate-reset, static-site-build-check, subagent-retrospective-guard, env-var-sample-consistency, curator-format, post-developer-format, stop-gate-failure-breaker, stop-spin-guard, context-factcheck-curator-stop | Non-build-modes-only: gate verdicts + retrospective validation. Dead under the loop (no SubagentStop fires for per-role `codegen-call` invocations) — the loop covers gate execution (`LoopGate`), formatting (`run_format_step`), cycle-state advancement, and (via `run_factcheck_step`) the context-curator factcheck scan explicitly as loop steps. `env-var-sample-consistency` fires for both developer roles, scanning the working-tree diff vs HEAD (relocated from a committer PreToolUse gate — committer cannot edit `.env.sample`, a structural deadlock). `context-factcheck-curator-stop` wraps `harnesses/claude/hooks/lib/context-factcheck-scan.sh` (shared with loop step); 3 claim classes: named-path, count-anchor, id-corruption (`_`→`*`, e.g. `register_route`; narrow, excludes space-`*`-space). Pi twin mirrors all 3, observe-only. |
...
Place **specific arms BEFORE wildcards** in shell case statements. Example: `INCONCLUSIVE:browser-not-installed)` before `INCONCLUSIVE:*)` in `static-site-build-check.sh` — first-match semantics ensure the specific handler wins and the wildcard doesn't swallow it.
