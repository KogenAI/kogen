...
| `harnesses/claude/hooks/dev-no-ci.sh` | PreToolUse — blocks developer from running CI commands (`mix test` bare-suite predicates go through `command_invokes()`, not a raw `^mix` anchor — an env-prefixed form like `MIX_ENV=test mix test --exclude slow` denies identically to the bare form) |
...
| `harnesses/claude/hooks/portable-launcher_test.sh` | Real launcher tests, both callers (`claude-shape/experiment`): CONTEXT_FLAGS/ROLE_SYSTEM_PROMPT append, drift-block, pitch resolution, Tier-0/Tier-1 loading via `pitch-context-selector.sh` (fail-open, citation priority, 6-row cap, missing-file error). Owns the Tier-0 `## Always Load` parser contract (Tests 16/17/21): bare-basename, backticked, `context/`-prefixed, and non-`.md` entries, on both launchers. |
