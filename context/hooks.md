...
| `harnesses/claude/hooks/planner-guard.sh`                       | PreToolUse — enforces planner constraints (no writes, no bash exec); blocks Bash redirects to `codegen/logging/*.md` when the command contains `git -C <abs-path>` (prevents transcript breakage from Bash tool_use entries that miss JSONL parsing gates)                                                                                  |
