...
**Deferred (not-yet-migrated):** `subagent-read-discipline.sh` (multi-line rationale) keeps a hand-authored header until a follow-up wave. The prior `role: unset` outliers (`context-index-parity.sh`, `context-file-size-gate.sh`, `context-factcheck-guard.sh`) were deleted — enforcement moved to `context-factcheck-edit-gate.sh` (writer's-turn gate) + the in-loop `run_curator_doc_check` step. Deferred entries live in `registry.yaml` as a commented-out NOT-YET-MIGRATED stanza in TWO places: (1) the registration section, (2) the header-block outlier comment. Keep both synchronized.
...
| `harnesses/claude/hooks/context-factcheck-edit-gate.sh` | PreToolUse (Edit\|Write\|MultiEdit, role: `*`) — denies ANY role's write to an orientation doc when the PROJECTED post-write content contains a factcheck violation (named-path claim, count-anchor mismatch, `_`→`*` identifier corruption); mirrors `curator-context-size-gate.sh`'s writer's-turn placement; supersedes the deleted `context-factcheck-curator-stop.sh` (dead SubagentStop) and `context-factcheck-guard.sh` (commit-time dead-end) |
...
`harnesses/claude/hooks/context-factcheck-edit-gate.sh` — PreToolUse: denies a write to any orientation doc whose PROJECTED post-write content fails `context-factcheck-scan.sh` (materializes projection into a throwaway git-init'd temp mirror, runs the unchanged scan lib). Writer's own turn — same placement rationale as `curator-context-size-gate.sh`.
...
## Retrospective Placement Rule (run_retrospective_step)

`subagent-retrospective-guard.sh` (SubagentStop hook) is dead-under-loop and deleted — replaced by `CodegenTestHarness.OrchestrationLoop.run_retrospective_step/3`, a post-role in-loop step firing after each developer/planner/reviewer handoff (`@retrospective_roles`: `developer-phoenix-backend`, `developer-phoenix-frontend`, `planner-phoenix`, `planner-static`, `reviewer-phoenix`, `reviewer-static` — mirrors the deleted hook's exact matcher set; `developer-static` was never covered by either).

The step asserts the role's returned envelope `value` contains a `### What I Learned This Step` header with at least one non-blank content line after it (`has_retrospective?/1`). Unlike the old hook, a missing block is NEVER fatal: a cold re-invoke cannot author an authentic retrospective (a fresh process has no memory of the work it did), so recovery instead warm-resumes the SAME session via `codegen-call --agent=<role> --resume=<session_id>` (the session id is threaded through `invoke_role/4`'s returned result), asking the still-warm role to add the omitted block. Bounded by `:max_retrospective_cycles` (default 1); any residual — no `session_id` available, a resume error, or the block still missing after the budget — soft-warns to stderr and proceeds. This step can never fail a build.
