...
| `harnesses/claude/hooks/planner-guard.sh`                       | PreToolUse — enforces planner constraints (no writes, no bash exec); blocks Bash redirects to `codegen/logging/*.md` including shell heredocs, `>>` appends, and brace-group redirect forms `{ ... } >> file` (all defeat transcript-based path detection; prevents transcript breakage); **implication**: session log version-stamps must be written via Edit tool, NOT bash redirect patterns of ANY form (`cat >> file <<EOF`, `{ ... } >>`, heredocs with `tee`, etc.) |
