...
| `SubagentStop` | developer-no-self-gate-reset | Non-build-modes-only: gate verdicts. Dead under the loop (no SubagentStop per-role) — loop covers gate exec (`LoopGate`), formatting (`run_format_step`), cycle-state advancement, env-var sample-consistency (`run_env_var_step` → `harnesses/claude/hooks/lib/env-var-sample-scan.sh`, diffs vs HEAD for **required** (no-default) reads only — `get_env("X")`/`fetch_env("X")` — flagged; defaulted reads exempt; ambient OS/shell vars (`HOME`,`PATH`,`LC_*`) skipped via allowlist (deployer never supplies via `.env`); relocated off committer, which cannot edit `.env.sample`), curator factcheck + index-parity (`run_curator_doc_check` → `context-factcheck-scan.sh` + `context-index-parity-scan.sh`; 3 claim classes: named-path, count-anchor, id-corruption). `curator-format`, `post-developer-format`, `static-site-build-check`, `stop-gate-failure-breaker`, `stop-spin-guard`, `context-factcheck-curator-stop` deleted (covered above). |
