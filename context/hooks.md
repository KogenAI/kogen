...
| `SubagentStop` | developer-no-self-gate-reset, context-factcheck-curator-stop | Non-build-modes-only: gate verdicts. Dead under the loop (no SubagentStop fires for per-role `codegen-call` invocations) — the loop covers gate execution (`LoopGate`), formatting (`run_format_step`), cycle-state advancement, env-var sample-consistency (via `run_env_var_step` shelling `harnesses/claude/hooks/lib/env-var-sample-scan.sh` — replaces the deleted `env-var-sample-consistency` hook, scanning the working-tree diff vs HEAD for undeclared env vars; relocated from a committer PreToolUse gate originally — committer cannot edit `.env.sample`, a structural deadlock), retrospective validation (via `run_retrospective_step` warm-resuming a role via `codegen-call --resume=<session_id>` to recover a missing `### What I Learned This Step` block — replaces the deleted `subagent-retrospective-guard` hook; never fails the build), and (via `run_factcheck_step`) the context-curator factcheck scan explicitly as loop steps. `curator-format`, `post-developer-format`, `static-site-build-check`, `stop-gate-failure-breaker`, and `stop-spin-guard` were deleted (dead-under-loop; each rule's live equivalent — `run_format_step`, `LoopGate`, `do_gate_loop`'s progress ceiling — already covers it). `context-factcheck-curator-stop` wraps `harnesses/claude/hooks/lib/context-factcheck-scan.sh` (shared with loop step); 3 claim classes: named-path, count-anchor, id-corruption (`_`→`*`, e.g. `register_route`; narrow, excludes space-`*`-space). Pi twin mirrors all 3, observe-only. |
