...
| `harnesses/claude/hooks/build-launcher-wrapper_test.sh` | Hermetic wrapper tests for claude-build.sh: CODEGEN_DIR 3-branch resolution, --queue dispatch, basename resolution, cwd normalization. ALL terminal execs stubbed. 13×2 launchers = 114 assertions. Sibling: `loop-signal-bridge_test.sh` covers the queue signal-teardown path. |
