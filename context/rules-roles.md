...
- **[local] Stuck-loop guard counts ALL developer spawns per step** — `developer-no-self-gate` cap (3 calls/session) spans whole step including interleaves. Passing gap-fix round can still trip spin-guard on spawn count. De-escalate via planner freshness check rather than re-run gate.
