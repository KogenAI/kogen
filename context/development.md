...
- **Test isolation scoping** — TypeScript: capture streams at test-body (not `beforeEach`/`afterEach`); restore in both paths. Env-var cleanup MUST run in BOTH `beforeEach` (first test clean) and `afterEach` (siblings clean); `afterEach`-only exposes Test 1 to ambient state. Bash: trap-clean temp dirs; PATH-stub binaries need per-line `PATH="$BIN_DIR:$PATH"`. Fixture layering: innermost clean → intermediate symlink → outermost dirty.
...
- **`make test` npm-ext transient race** — The npm-extension block runs `npm run build` + `npm test` in serial per extension, but all extensions execute in a background subshell in parallel with hooks/install/scaffold/subagents tasks. Enforcement extension's tsc build writes to `dist/` while its test runner simultaneously imports from `dist/` — slow machines may see transient "module not found" or partial-file read. One-time gate failure → re-run passes. No source fix needed; not a durable failure.
- **`templates/generator/install_test.sh` not auto-discovered** — Bash test files under `test_harness/install/` are auto-discovered by `run-tests.sh`; `templates/generator/install_test.sh` lives outside that dir and is NOT auto-discovered. Test runs when called directly; `make test` skips it. Structural-grep tests in both dirs must reconcile their sentinels (e.g., var names for `install.sh` markers) when editing install.sh's output. Both dirs' tests pass independently if run directly; conflict surfaces only on full `make test` when both hooks auto-run.
- **Session-log-no-duplicate-section hook blocks Edit if old/new contains `## <role> Section`** — The hook denies any Edit to session logs whose `old_string` or `new_string` contains the pattern `## <role> Section` (e.g., `## developer-phoenix-backend Section`), even when editing body content under an already-present header. The guard cannot distinguish between removing a duplicate header vs editing nearby body. Workaround: Use Write tool to rewrite the full log file instead of Edit.
...
- **Bash 3.2 compatibility** — No `declare -A` (use indexed array), no `wait -n`. `git -C <nonexistent-dir>` exits 128 — walk to ancestor; use `hooks_realpath` (macOS symlink issue).
...
- **Heredoc expansion** — unquoted `<<EOF` expands variables; `<<'EOF'` does not. When using multiple heredocs in test stubs (e.g., fake-git branches), match existing quoting convention: `<<'HEREDOC'` (single-quoted) uses bare `$*`; unquoted `<<HEREDOC` requires `\$*`. New lines added to a stub must respect the stub's existing convention.
