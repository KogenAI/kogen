...
- **Test source change requires migrating ALL dependent cases (Rule J)** — When a fixture's input source changes, all test cases using the old path must migrate; leaving them asserts the OLD spec (dead code). Non-retryable/allow-only cases producing the same verdict regardless can stay. Example: prose-based classification dropped → re-wire ALL retryable block-verdict cases to flagged transcript records; allow-cases can stay prose-only.
...
- **Dual-read unset-case Bash test must unset BOTH vars** — `env -u NEW -u OLD bash "$HOOK"`. Single unset leaves fallback active.
...
- **Pi test cleanup via delete must cover both old and new names** — cleanup/unset paths must delete both `NEW_NAME` and `OLD_NAME`. Single cleanup leaves fallback active.
...
- **Operator toggles vs app runtime config** — `DEPLOY_AUTO=1`, `RELEASE_DAY_OVERRIDE=1`, `codegen-scaffold --recipe-source=<path>` are operator toggles → NOT in `.env.sample`/`.env.prod.sample`. Operator toggles go in script header comments; app env vars go in `.env.sample`. Operator toggles control harness/build behavior; app env vars control app runtime.
- **Deriving state properties from ordered lists** — Derive computed property (e.g., "terminal" state) from the list's LAST element: `for w in $CYCLE_STATE_ORDER; do last="$w"; done; [ "$1" = "$last" ]` — not hard-coded `[ "$1" = "COMMITTED" ]`. Adding a new state after the current terminal shifts the property automatically. Corollary: callers test returned values (`[ -n "$result" ]`); helpers return empty string on unmatched input (`set -u`-safe).
