...
- **Pitch byte targets grow stale** — Planner must re-measure `wc -c` at plan time (not pitch time) to recalculate headroom; stale budgets flip optional moves into mandatory reallocations, causing gate failures.
...
- **Curator learning silently dropped at 40,960-byte cap** — curator-edited `context/*.md` exceeding the byte cap gets reverted by `context-file-size-gate.sh`, losing edits. Before finishing, check `wc -c`; if over 40,960 B, compress stale bullets, relocate examples, or split to new file. Curator now owns the cap.
