...
- **prompt-content-parity_test.sh sentinel sync** — When a rule file or tools-header source file (e.g., `harnesses/claude/tools-header/build.txt`, `harnesses/pi/tools-header/build.txt`) changes load-bearing text, parity-test sentinels MUST be updated to match exactly. Assertions use `grep -qF` (fixed-string grep). Sentinels must target SOURCE files, not baked install-destination paths. **Critical**: tools-header edits require `make install` to bake into `*-build-system-prompt.txt` BEFORE the parity test can pass — runner calls `make install` automatically before running prompt-content-parity_test.sh. **Byte-identity across dual sources**: when a single sentinel must verify identical text in both harnesses (e.g., trigger bullet in both claude and pi build.txt), use one SENTINEL variable containing the unique phrase and assert it in BOTH baked prompts via separate `assert_contains` calls. **Pre-validation**: grep `prompt-content-parity_test.sh` for source-file names being added; zero hits → no sentinel sync needed. **Sentinel retargeting when content relocates**: retarget the parity-test file-path arg to the new location — sentinel STRINGS remain unchanged when content moves verbatim. **Example**: editing `shared/rules/roles/planner.md` Step 0 prose without changing sentinel-matching strings (checked via `grep prompt-content-parity_test.sh` for "planner.md" or "Step 0"): zero parity sentinels target that file → no sentinel edits required (drift detection is skipped for rules without sentinels).
...
- **Context files carry a 40 KB advisory cap** — `context/*.md` domain files have a ~40,960-byte (40 KB) advisory size limit. No enforced hook exists; constraint is advisory to guide curation load-balancing across the context-file suite. When a file approaches cap, compress redundancy or relocate verbose examples to another context file. **CURRENT EXCEEDANCES**: `context/harnesses.md` is 41,899 bytes (939 bytes over cap). Once the `context-file-size-gate` hook ships, the next commit touching harnesses.md will be blocked until it is trimmed. Two files are near-cap: `test-coverage.md` (40,623 B) and `hook-authoring-patterns.md` (40,653 B) — small additions will trigger the gate. Plan trim/split work if future commits require touching these files.
- **Pitch byte targets grow stale** — When a content file grows between shaping and implementation, pre-measured byte budgets (in the pitch) no longer reflect live sizes. Planner MUST re-measure `wc -c` on source files at plan time (not pitch time) and recalculate headroom. Off-by-one provisional budgets can flip optional moves into mandatory reallocations; trusting stale numbers leads to gate failures at dev time.
- **Heredoc piping with `>` redirects trips planner-guard** — `python3 - <<EOF ... > output.txt ... EOF` (a `>` redirect INSIDE the heredoc body) is detected by planner-guard as a suspicious output redirect. Workaround: write the script to a temp file (`cat > /tmp/script.py <<EOF ... EOF`) then `python3 /tmp/script.py > output.txt` (redirect OUTSIDE the subagent prompt). Separates probe logic from output handling.
- **Prettier 3.8 re-pads wide-cell markdown tables** — markdown tables with long cell values (e.g., Purpose columns in inventory tables) are re-padded to full alignment by `prettier` even when using minimal-separator syntax (`| - |`). The only reliable guard for byte-capped context files is `.prettierignore`: add the targeted context files to `.prettierignore` BEFORE `make format`. Any future context-file split with wide tables must add `.prettierignore` entries or use different table structure (e.g., definition lists).
...
- **Curator learning silently dropped at 40,960-byte cap** — if a curator-edited `context/*.md` exceeds the byte cap, `context-file-size-gate.sh` denies the commit and the file is reverted, silently losing the curator's edits. The curator now owns the cap: before finishing, check projected size (`wc -c`); if over 40,960 B, compress a stale bullet, relocate a verbose example, or split to a new context file. The `curator-learning-committed` gate at `BUILD_RESULT: success` blocks if a curator-recorded file is missing from HEAD.
