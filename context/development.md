...
- **Prose-fix planning: anchor to exact text, not line numbers**: When drafting a plan that specifies edits to documentation (pitch files, context docs, etc.), always cite the exact sentence or phrase to be edited — never line numbers. Line numbers drift as files evolve; future developers reading the plan may work against a stale version and miss the intended target. Anchor examples: "the sentence starting with 'AGENTS.md and CLAUDE.md at root'", "the Notable Artifacts bullet about `templates/AGENTS-HYBRID.md.j2`". This principle applies to both planning documents and reviewer feedback; developers executing the plan use these anchors with the Read and Edit tools to ensure surgical accuracy across time.
