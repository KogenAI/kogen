...
**Promote drafts when splitting them.** If source document path is under `codegen/designs/drafts/`, after rewriting the steps section, move the file: `mv codegen/designs/drafts/<slug>.md codegen/designs/ready/<slug>.md`. The move IS the promotion signal — `drafts/` = still being shaped, `ready/` = step list exists, implementation can start. Mention the new path in the reply. Archival from `ready/` to `codegen/designs/archive/` is manual and only triggered by user instruction to `pi-build` after work ships — never archive from `/split`.
