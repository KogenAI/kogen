...
4. **Save automatically** — save with a descriptive kebab-case filename. Never ask for path. `mkdir -p codegen/designs/drafts` then save to `codegen/designs/drafts/<slug>.md`, regardless of launcher or `CLAUDE_ROLE`. Re-running on an existing slug → Edit in place; the doc is a living spec. Promotion to `codegen/designs/ready/` happens via `/split` when the design is implementation-ready; archival from `ready/` to `codegen/designs/archive/` is manual (ask `claude-build` when work is shipped).
