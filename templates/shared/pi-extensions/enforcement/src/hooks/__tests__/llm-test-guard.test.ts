...
describe("llm-test-guard", () => {
  ...
  it("blocks mix test --only llm_integration with only PARTITION=1 set", async () => {
    const result = await runHook("MIX_TEST_PARTITION=1 mix test --only llm_integration test/foo_test.exs");
    assert.ok((result as { block?: boolean }).block === true);
  });

  it("blocks mix test --only llm_integration PARTITION=3 PARTITIONS=10", async () => {
    const result = await runHook("MIX_TEST_PARTITION=3 MIX_TEST_PARTITIONS=10 mix test --only llm_integration test/foo_test.exs");
    assert.ok((result as { block?: boolean }).block === true);
  });

  it("allows PARTITION=1 PARTITIONS=1 with .exs path", async () => {
    const result = await runHook("MIX_TEST_PARTITION=1 MIX_TEST_PARTITIONS=1 mix test --only llm_integration test/combobulate/llm_integration/foo_test.exs");
  ...
  it("blocks PARTITION=1 PARTITIONS=1 without .exs path", async () => {
    const result = await runHook("MIX_TEST_PARTITION=1 MIX_TEST_PARTITIONS=1 mix test --only llm_integration");
    assert.ok((result as { block?: boolean }).block === true);
  });
});
