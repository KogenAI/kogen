#!/bin/bash
# _role.sh — shared helper: resolve the active role across all three harnesses.
#
...
# CLAUDE_ROLE (Claude Code), PI_ROLE (future PI harness), and CODEX_ROLE
# (Codex) with a consistent precedence order.
#
...
resolve_role() {
    for v in "${CLAUDE_ROLE:-}" "${PI_ROLE:-}" "${CODEX_ROLE:-}"; do
        [ -n "$v" ] && {
            printf '%s' "$v"
            return 0
        }
