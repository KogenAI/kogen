...
role=$(resolve_role)
debug_log debug-bash-safety-guard "tool=$TOOL_NAME agent=$AGENT_TYPE role=${role} cmd=$COMMAND"
...
if printf '%s' "$COMMAND" | grep -qE '\brm[[:space:]].*-[a-zA-Z]*r[a-zA-Z]*[[:space:]]|\brm[[:space:]].*--recursive\b'; then
    deny "BLOCKED by debug-bash-safety-guard: recursive rm forbidden in ${role} sessions (read-only investigation)"
    exit 0
fi
...
if printf '%s' "$COMMAND" | grep -qE '\bmix[[:space:]]+(ecto\.(migrate|rollback|drop|reset|create)|ecto\.setup)\b'; then
    deny "BLOCKED by debug-bash-safety-guard: DB migration/drop commands forbidden in ${role} sessions"
    exit 0
fi
...
if printf '%s' "$COMMAND" | grep -qE '\bgit[[:space:]]+(commit|push)\b'; then
    deny "BLOCKED by debug-bash-safety-guard: git write operations forbidden in ${role} sessions"
    exit 0
fi

if printf '%s' "$COMMAND" | grep -qE '\bgit[[:space:]]+reset\b.*--hard\b'; then
    deny "BLOCKED by debug-bash-safety-guard: git reset --hard forbidden in ${role} sessions"
    exit 0
fi
...
if printf '%s' "$COMMAND" | grep -qE '\bmix[[:space:]]+deps\.get\b'; then
    deny "BLOCKED by debug-bash-safety-guard: mix deps.get modifies mix.lock — forbidden in ${role} sessions"
    exit 0
fi

# mix run priv/repo/seeds (mutates DB)
if printf '%s' "$COMMAND" | grep -qE '\bmix[[:space:]]+run\b.*seeds\b'; then
    deny "BLOCKED by debug-bash-safety-guard: running seeds mutates DB — forbidden in ${role} sessions"
    exit 0
fi
...
if printf '%s' "$COMMAND" | grep -qiE '\b(TRUNCATE|DROP[[:space:]]+TABLE|DELETE[[:space:]]+FROM)\b'; then
    deny "BLOCKED by debug-bash-safety-guard: destructive SQL (TRUNCATE/DROP TABLE/DELETE FROM) forbidden in ${role} sessions"
    exit 0
fi

# Destructive HTTP (curl -X DELETE, POST, PUT, PATCH)
if printf '%s' "$COMMAND" | grep -qE 'curl[[:space:]].*-X[[:space:]]+(DELETE|POST|PUT|PATCH)'; then
    deny "BLOCKED by debug-bash-safety-guard: destructive HTTP (curl -X DELETE/POST/PUT/PATCH) forbidden in ${role} mode — investigation only"
    exit 0
fi

# Docker mutations
if printf '%s' "$COMMAND" | grep -qE '\bdocker[[:space:]]+(run|exec|kill|rm|stop|start)\b'; then
    deny "BLOCKED by debug-bash-safety-guard: docker mutations forbidden in ${role} mode — investigation only"
    exit 0
fi
...
if printf '%s' "$COMMAND" | grep -qE '\b(systemctl|launchctl)[[:space:]]+(start|stop|restart|reload|enable|disable)\b'; then
    deny "BLOCKED by debug-bash-safety-guard: service control forbidden in ${role} mode — investigation only"
    exit 0
fi

# kill/pkill/killall
if printf '%s' "$COMMAND" | grep -qE '\b(kill|pkill|killall)\b'; then
    deny "BLOCKED by debug-bash-safety-guard: kill forbidden in ${role} mode — use journalctl/ps for inspection only"
    exit 0
fi
...
if printf '%s' "$COMMAND" | grep -qE '\b(npm|pip|pip3|brew|gem|pnpm|cargo)[[:space:]]+(install|add|update|upgrade|remove)\b'; then
    deny "BLOCKED by debug-bash-safety-guard: package installation forbidden in ${role} mode — investigation only"
    exit 0
fi
