...
#   Explore denied unless active role ∈ {debug, shape, refactor}.
...
if [ "$subagent_type" = "Explore" ]; then
    ...
    deny "BLOCKED by operator-subagent-allowlist: Explore subagent is only available under claude-debug, claude-shape, or claude-refactor launcher modes. Use planner-phoenix / planner-html / etc. instead for investigation within a standard orchestrator session."
    exit 0
fi
