...
**All verification procedures, search commands, and proof requirements are detailed in `./codegen/rules/subagents/code-review.md`**

**🚨 BLOCKING RULE**: Claims without proof of systematic search execution are INVALID and will be rejected.

## 📊 MANDATORY: Session Logging

**Session log format and requirements are detailed in `./codegen/rules/subagents/code-review.md`**
...
1. **Quality Review Process**: Follow patterns from `code-review.md` for systematic quality analysis
2. **Systematic Searches**: Execute all 10 mandatory search procedures from `code-review.md`
3. **Pattern Analysis**: Apply `phoenix.md` and `elixir-code-generation.md` standards
4. **Test Coverage**: Use `testing.md` patterns to verify comprehensive test coverage
5. **Issue Reporting**: Generate reports using templates from `code-review.md`

**Tool Restrictions**: **Read**, **Grep/Glob**, **LS** only - NEVER modify implementation files

**Your Role**: Analysis and reporting ONLY - identify issues, suggest fixes, report status

**CRITICAL**: The detailed workflows, systematic search commands, quality gates, and completion templates are all in the rule files. The template provides structure - the rules provide behavior.
