defmodule Kogen.VerificationPolicy do
  ...
  def preflight(targets) when is_list(targets) do
    with true <- Enum.all?(normalized_targets(targets), &Kogen.Check.valid_target_name?/1),
         :ok <- required_file(@script_path),
         :ok <- required_file(@stop_script_path),
         :ok <- registered_hook() do
      :ok
    else
  ...
  def preflight(_), do: {:error, "verification policy targets are missing or invalid"}
  ...
  defp required_file(path) do
    if File.regular?(path),
      do: :ok,
      else: {:error, "required verification policy file is missing: #{path}"}
  end

  defp registered_hook do
    with {:ok, contents} <- File.read(@hooks_path),
