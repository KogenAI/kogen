...
`process_template.py` recognizes two `tool.name` values: `claude`, `codex`. Two branches render differently — not future hooks, real divergence today.

**Mechanics** (`process_template.py` `_strip_template_blocks`, regex pass):

- `{% if tool.name == 'claude' %}A{% else %}B{% endif %}` → claude keeps `A`, codex keeps `B`.
- `{% if tool.name == 'claude' %}A{% endif %}` (no else) → claude keeps `A`, codex drops entire block.
...
- Codex (AGENTS.md) does NOT auto-resolve `@`-references → else branch must instruct the agent explicitly: `→ See \`<path>\` for <reason>` so the agent Reads on demand.

Same intent, different delivery mechanism. Don't collapse to one branch — codex agents would silently miss the file.
...
- `process_template.py` is the single rendering engine. It supports two output formats:
  - `--format=md` (default) — Markdown body for Claude.
