#!/usr/bin/env python3
"""The small, native-only Codex runtime installer used by Kogen's Elixir layer.
...
def platform_name(machine: Optional[str] = None, system: Optional[str] = None) -> str:
    ...
    if system != "darwin":
        raise InstallerError("managed Codex runtime is currently available only on macOS")
...
def _native_triple(name: str) -> str:
    return {"darwin-arm64": "aarch64-apple-darwin", "darwin-x64": "x86_64-apple-darwin"}[name]
...
def _official_get(url: str) -> bytes:
    ...
    if parsed.scheme != "https" or parsed.hostname != "registry.npmjs.org" or parsed.port is not None:
        raise InstallerError("official Codex download must use https://registry.npmjs.org")

    class NoRedirect(urllib.request.HTTPRedirectHandler):
        def redirect_request(self, req, fp, code, msg, headers, newurl):
            raise InstallerError("official Codex download redirected to an untrusted location")
    ...
    raise InstallerError("could not download official Codex artifact after 3 attempts: " + str(last_error)) from last_error
...
def _runtime(root: Path, version: str, platform: str) -> Path:
    if not version or "/" in version or "\\" in version or version in {".", ".."}:
        raise InstallerError("invalid Codex version")
...
def _required_paths(platform: str) -> tuple[str, tuple[str, ...]]:
    vendor = "vendor/" + _native_triple(platform) + "/"
    return vendor + "bin/codex", (
        vendor + "bin/codex-code-mode-host",
        vendor + "codex-path/rg",
        vendor + "codex-resources/zsh/bin/zsh",
        vendor + "codex-package.json",
    )


def _validate_descriptor(runtime: Path, descriptor_path: str) -> None:
    try:
        descriptor = json.loads((runtime / descriptor_path).read_text(encoding="utf-8"))
    except (OSError, ValueError) as error:
        raise InstallerError("invalid native Codex package descriptor") from error
    if not isinstance(descriptor, dict):
        raise InstallerError("invalid native Codex package descriptor")
...
def _write_manifest(runtime: Path, version: str, platform: str) -> Dict[str, Any]:
    ...
    _validate_descriptor(runtime, resources[-1])
...
def _verify_directory(runtime: Path, version: str, platform: str) -> Dict[str, Any]:
    ...
    if any(not (actual[item]["mode"] & 0o111) for item in (executable, *resources[:-1])):
        raise InstallerError("managed Codex native resources are not executable")
...
def _native_check(runtime: Dict[str, Any]) -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="kogen-codex-probe-") as private:
        environment = {"HOME": private, "CODEX_HOME": private, "PATH": "/usr/bin:/bin", "NO_COLOR": "1"}
        for args in ([executable, "--version"], [executable, "login", "--help"]):
            try:
                completed = subprocess.run(args, stdin=subprocess.DEVNULL, stdout=subprocess.DEVNULL,
                                           stderr=subprocess.PIPE, text=True, timeout=30, check=False, env=environment)
            except OSError as error:
                raise InstallerError("could not launch validated native Codex runtime: " + str(error)) from error
            if completed.returncode != 0:
                raise InstallerError("validated native Codex runtime rejected %s: %s" % (" ".join(args[1:]), completed.stderr.strip()))
...
def _platform_version(version: str, platform: str) -> str:
...
def _matches_platform(metadata: Dict[str, Any], version: str, platform: str) -> bool:
    ...
    return (metadata.get("name") == "@openai/codex" and metadata.get("version") == _platform_version(version, platform)
...
def stage(root: Path | str, version: str, *, platform: Optional[str] = None,
    ...
    try:
        ...
        # The two native probes happen only after integrity, archive type/path,
