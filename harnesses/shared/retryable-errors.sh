...
# Defines four regexes consumed by grep -qE. Harness-agnostic: the
...
# retryable_regex above (no overlap by design).
switch_model_regex='model.*unavailable|provider.*unavailable|model.*disabled|model.*not found|unknown model|is currently unavailable'
