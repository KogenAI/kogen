...
# worktree_destroy <slug> — idempotent teardown (folded experiment_prune).
# Removes .claude/worktrees/exp-<slug>, deletes branch worktree-exp-<slug>,
# releases the Phoenix port (project=basename(PWD), workspace=exp-<slug>), and
# runs `git worktree prune`. Every step fail-open so re-running or partial
# state never errors.
#
