...
function formatSkillSourceCounts(
  skills: Array<{ source: SkillSource }>,
): string {
...
function formatConfiguredSessionDir(input: DoctorReportInput): string {
  if (input.requestedSessionDir) {
    return path.resolve(
      input.expandTilde?.(input.requestedSessionDir) ??
        input.requestedSessionDir,
    );
  }
  if (input.config.defaultSessionDir) {
    return path.resolve(
      input.expandTilde?.(input.config.defaultSessionDir) ??
...
function formatIntercomDiagnostic(
  ...
  if (diagnostic.intercomConfigError) {
    lines.push(
      `- intercom config warning: ${diagnostic.intercomConfigError}; runtime assumes enabled`,
    );
  }
  return lines;
}
