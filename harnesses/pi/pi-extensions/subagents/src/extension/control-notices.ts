...
export function controlNoticeTarget(
...
export function clearPendingForegroundControlNotices(
  state: SubagentState,
  runId?: string,
): void {
...
function isForegroundNoticeStillActionable(
  ...
  if (control.currentAgent && control.currentAgent !== details.event.agent)
    return false;
  if (
    details.event.index !== undefined &&
    control.currentIndex !== details.event.index
  )
    return false;
...
export function handleSubagentControlNotice(input: {
  ...
  const pending =
    input.state.pendingForegroundControlNotices ??
