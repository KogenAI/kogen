...
function startNestedControlInboxListener(
  ...
  const pendingResults = new Map<
    string,
    Parameters<typeof writeNestedControlResult>[1]
  >();
  const timer = setInterval(() => {
    try {
      for (const request of readNestedControlRequests(route)) {
        if (seen.has(request.requestId) || inFlight.has(request.requestId))
          continue;
        ...
        void (async () => {
          try {
            ...
            if (!result) {
              ...
              try {
                const control = state.foregroundControls.get(
                  request.targetRunId,
                );
                if (!control) {
