export const PROMPT_TEMPLATE_SUBAGENT_REQUEST_EVENT =
  "prompt-template:subagent:request";
export const PROMPT_TEMPLATE_SUBAGENT_STARTED_EVENT =
  "prompt-template:subagent:started";
export const PROMPT_TEMPLATE_SUBAGENT_RESPONSE_EVENT =
  "prompt-template:subagent:response";
export const PROMPT_TEMPLATE_SUBAGENT_UPDATE_EVENT =
  "prompt-template:subagent:update";
export const PROMPT_TEMPLATE_SUBAGENT_CANCEL_EVENT =
...
function parsePromptTemplateRequest(
  data: unknown,
): PromptTemplateDelegationRequest | undefined {
...
function filterRecentOutput(lines: string[] | undefined): string[] | undefined {
  ...
  const filtered = lines.filter(
    (line) =>
      typeof line === "string" && line.trim() && line.trim() !== "(running...)",
  );
...
function resolveProgressModel(
  ...
  if (entry.agent) {
    const byAgent = results.find(
      (result) =>
        result.agent === entry.agent && typeof result.model === "string",
    );
...
export function registerPromptTemplateDelegationBridge<
  Ctx extends { cwd?: string },
>(
  ...
  subscribe(PROMPT_TEMPLATE_SUBAGENT_REQUEST_EVENT, async (data) => {
    ...
    if (!ctx) {
      const response: PromptTemplateDelegationResponse = {
        ...
        isError: true,
        errorText:
          "No active extension context for delegated subagent execution.",
      };
    ...
    try {
      ...
      const parallelResults = request.tasks
        ? request.tasks.map<PromptTemplateDelegationParallelResult>(
            (task, index) => {
              ...
              return {
                ...
                isError:
                  (exitCode !== undefined && exitCode !== 0) || !!errorText,
