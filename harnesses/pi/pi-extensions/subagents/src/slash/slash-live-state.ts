...
function buildParallelInitialResult(
  ...
  return {
    content: [
      {
        type: "text",
        text: tasks.map((task) => `${task.agent}: ${task.task}`).join("\n\n"),
      },
    ],
    details: {
      mode: "parallel",
...
function isParallelChainStep(
  step: ChainStepLike,
): step is ParallelChainStepLike {
...
function cloneResultsWithProgress(
  ...
  return results.map((result, index) => {
    const nextProgress =
      progress?.find((entry) => entry.index === index) ??
      progress?.[index] ??
      result.progress;
...
export function applySlashUpdate(
  ...
  const nextDetails: Details = {
    ...
    progress,
    results: cloneResultsWithProgress(
      snapshot.result.details.results,
      progress,
    ),
...
export function failSlashResult(
  ...
  const failedResults = initial.details.results.map((result) => ({
    ...
    progress: result.progress
      ? { ...result.progress, status: "failed" as const }
      : result.progress,
  }));
...
export function resolveSlashMessageDetails(
  value: unknown,
): SlashMessageDetails | undefined {
...
export function restoreSlashFinalSnapshots(entries: unknown[]): void {
  ...
  for (const entry of entries) {
    ...
    if (!details) continue;
    finalSnapshots.set(details.requestId, {
      result: details.result,
      version: nextVersion(),
    });
  }
