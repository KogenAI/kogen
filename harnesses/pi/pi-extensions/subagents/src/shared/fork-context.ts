  ...
  return {
    sessionFileForIndex(index = 0): string | undefined {
      ...
      try {
        ...
        if (!sessionFile) {
          throw new Error(
            "Session manager did not return a forked session file.",
          );
        }
        if (!fs.existsSync(sessionFile)) {
          throw new Error(
            `Session manager returned a forked session file that does not exist: ${sessionFile}`,
          );
        }
        ...
        throw new Error(
          `Failed to create forked subagent session: ${cause.message}`,
          { cause },
        );
      }
    },
