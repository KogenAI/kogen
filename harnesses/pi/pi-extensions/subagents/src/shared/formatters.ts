...
export function formatModelThinking(model?: string, thinking?: string): string {
  ...
  const explicitThinking = THINKING_LEVELS.find(
    (level) => level === thinking?.trim(),
  );
  ...
  return [
    displayModel,
    displayThinking ? `thinking ${displayThinking}` : undefined,
  ]
    .filter(Boolean)
    .join(" · ");
}
...
export function buildChainSummary(
  ...
  const stepNames = steps
    .map((step) =>
      isParallelStep(step) ? `parallel[${step.parallel.length}]` : step.agent,
    )
    .join(" → ");

  const totalDuration = results.reduce(
    (sum, r) => sum + (r.progress?.durationMs || 0),
    0,
  );
  ...
  const skillsLine =
    allSkills.size > 0 ? `🔧 Skills: ${[...allSkills].join(", ")}` : "";
