...
export function createResultWatcher(
  ...
  const handleResult = async (file: string) => {
    ...
    try {
      const data = JSON.parse(
        fsApi.readFileSync(resultPath, "utf-8"),
      ) as ResultFileData;
      if (data.sessionId && data.sessionId !== state.currentSessionId) return;
      if (
        !data.sessionId &&
        data.cwd &&
        (!state.baseCwd || data.cwd !== state.baseCwd)
      )
        return;
      ...
      const normalizedChildren = attachNestedChildrenToResultChildren(
        runId,
        resultChildren.map(
          (result = {}, index): SubagentResultIntercomChild => {
            const baseOutput = result.output ?? data.summary;
            const hasRealOutput =
              typeof baseOutput === "string" && baseOutput.trim().length > 0;
            ...
            const sessionPath =
              result.sessionFile ??
              (resultChildren.length === 1 ? data.sessionFile : undefined);
            ...
            return {
              ...
              status: resolveSubagentResultStatus({
                ...
                state:
                  data.state === "paused" || typeof result.success !== "boolean"
                    ? data.state
                    : undefined,
              }),
              summary,
              ...
              fsApi.existsSync(sessionPath)
                ? { sessionPath }
                : {}),
              ...(result.intercomTarget
      ...
      console.error(
        `Failed to process subagent result file '${resultPath}':`,
        error,
      );
    }
  };
  ...
  const primeExistingResults = () => {
    try {
      ...
      if (isNotFoundError(error)) return;
      console.error(
        `Failed to scan subagent result directory '${resultsDir}':`,
        error,
      );
    }
  };
  ...
  const scheduleRestart = () => {
    ...
    state.watcherRestartTimer = timers.setTimeout(() => {
      ...
      try {
        ...
        console.error(
          `Failed to restart subagent result watcher for '${resultsDir}':`,
          error,
        );
        scheduleRestart();
      }
  ...
  const startResultWatcher = () => {
    ...
    try {
      ...
      state.watcher.on("error", (error) => {
        ...
        console.error(
          `Subagent result watcher failed for '${resultsDir}':`,
          error,
        );
        state.watcher?.close();
        state.watcher = null;
      ...
      console.error(
        `Failed to start subagent result watcher for '${resultsDir}':`,
        error,
      );
      state.watcher = null;
      scheduleRestart();
