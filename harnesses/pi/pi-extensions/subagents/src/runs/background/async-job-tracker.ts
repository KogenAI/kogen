...
export function createAsyncJobTracker(
  ...
  const rerenderWidget = (
    ...
    jobs = Array.from(state.asyncJobs.values()),
  ) => {
  ...
  const emitNewControlEvents = (job: AsyncJobState) => {
    ...
    try {
      ...
      console.error(
        `Failed to open async control events for '${job.asyncDir}':`,
        error,
      );
      return;
    }
    try {
      ...
      const cursor =
        stat.size < (job.controlEventCursor ?? 0)
          ? 0
      ...
      for (const line of buffer
        ...
        try {
          ...
          console.error(
            `Ignoring malformed async control event in '${eventsPath}':`,
            error,
          );
          continue;
        }
      ...
      console.error(
        `Failed to read async control events for '${job.asyncDir}':`,
        error,
      );
    } finally {
      fs.closeSync(fd);
        ...
        try {
          ...
          if (status) {
            ...
            if (status.steps?.length) {
              ...
              job.hasParallelGroups =
                groups.length > 0 || job.hasParallelGroups;
              const activeGroup =
              ...
              job.runningSteps = visibleSteps.filter(
          ...
          if (job.status !== "failed") {
            console.error(
              `Failed to read async status for '${job.asyncDir}':`,
              error,
            );
            job.status = "failed";
            job.updatedAt = Date.now();
  ...
  const handleStarted = (data: unknown) => {
    ...
    const agents =
      firstGroupCount && firstGroupCount > 0
