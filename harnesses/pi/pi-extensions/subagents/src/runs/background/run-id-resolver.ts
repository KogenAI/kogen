...
import {
  ...
  type AsyncRunLocation,
} from "./async-resume.ts";
import {
  assertSafeNestedId,
...
function nestedScopeFromState(
  ...
  for (const job of state.asyncJobs.values())
    add(job.nestedRoute as NestedRoute | undefined);
  return { routes };
}
...
export function resolveSubagentRunId(
  ...
  const exactNested = findNestedRunMatchesById(
    id,
  ...
  const unique = new Map(
    matches.map((match) => [`${match.kind}:${match.id}`, match]),
  );
