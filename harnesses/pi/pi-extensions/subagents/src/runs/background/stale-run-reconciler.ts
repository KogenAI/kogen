...
function readStatusFile(asyncDir: string): AsyncStatus | null {
  ...
  try {
    ...
    throw new Error(
      `Failed to read async status file '${statusPath}': ${getErrorMessage(error)}`,
      {
    ...
    return {
      ...
      durationMs:
        step.startedAt !== undefined && step.durationMs === undefined
          ...
          : step.durationMs,
      exitCode:
        step.exitCode ?? (state === "complete" || state === "paused" ? 0 : 1),
...
function buildFailedRepair(
  ...
  const resultAgent =
    repairedSteps[status.currentStep ?? 0]?.agent ??
    repairedSteps[0]?.agent ??
    "subagent";
  return {
...
export function reconcileAsyncRun(
  ...
  if (fs.existsSync(resultPath)) {
    ...
    if (terminalStatus) {
      ...
      return {
        ...
        resultPath,
        message:
          "Existing async result file was used to repair stale running status.",
      };
    }
  ...
  if (!status) {
    const startedAt =
      options.startedRun?.startedAt ?? effectiveStatus.startedAt;
