...
function ensureObject(value: unknown, source: string): Record<string, unknown> {
  if (!value || typeof value !== "object" || Array.isArray(value)) {
    throw new Error(
      `Async result file '${source}' must contain a JSON object.`,
    );
  }
...
function validateOptionalString(
  ...
  if (typeof fieldValue !== "string")
    throw new Error(
      `Invalid async result file '${source}': ${displayField} must be a string.`,
    );
  return fieldValue;
}
...
function validateResultFile(
  ...
  if (resultsValue !== undefined) {
    if (!Array.isArray(resultsValue))
      throw new Error(
        `Invalid async result file '${resultPath}': results must be an array.`,
      );
    results = resultsValue.map((entry, index) => {
      const child = ensureObject(entry, `${resultPath} results[${index}]`);
      const agent = validateOptionalString(
        child,
        "agent",
        resultPath,
        `results[${index}].agent`,
  ...
  if (success !== undefined && typeof success !== "boolean")
    throw new Error(
      `Invalid async result file '${resultPath}': success must be a boolean.`,
    );
  return {
...
function readResultFile(resultPath: string): AsyncResultFile {
  ...
  try {
    ...
    throw new Error(
      `Failed to read async result file '${resultPath}': ${getErrorMessage(error)}`,
      {
...
function prefixedRunIds(dir: string, prefix: string, suffix = ""): string[] {
  ...
  return fs
    ...
    .filter(
      (entry) =>
        entry.startsWith(prefix) && (!suffix || entry.endsWith(suffix)),
    )
...
export function resolveAsyncRunLocation(
  ...
  const resultRoot = path.resolve(resultsDir);
  const requestedId =
    assertRunId(params.id, "id") ?? assertRunId(params.runId, "runId");
  if (params.dir) {
  ...
  const matching = findAsyncRunPrefixMatches(
    requestedId,
    asyncRoot,
    resultRoot,
  );
...
function validateStatusForResume(
  ...
  if (typeof status.runId !== "string")
    throw new Error(
      `Invalid async status '${source}': runId must be a string.`,
    );
  if (status.sessionId !== undefined && typeof status.sessionId !== "string")
    throw new Error(
      `Invalid async status '${source}': sessionId must be a string.`,
    );
  ...
  if (
    ...
    throw new Error(
      `Invalid async status '${source}': sessionFile must be a string.`,
    );
  if (status.steps !== undefined) {
    if (!Array.isArray(status.steps))
      throw new Error(
        `Invalid async status '${source}': steps must be an array.`,
      );
    status.steps.forEach((step, index) => {
      if (!step || typeof step !== "object" || Array.isArray(step))
        throw new Error(
          `Invalid async status '${source}': steps[${index}] must be an object.`,
        );
...
function validateResumeSessionFile(runId: string, sessionFile: string): string {
  if (path.extname(sessionFile) !== ".jsonl")
    throw new Error(
      `Async run '${runId}' session file must be a .jsonl file: ${sessionFile}`,
    );
  const resolved = path.resolve(sessionFile);
  if (!fs.existsSync(resolved))
    throw new Error(
      `Async run '${runId}' session file does not exist: ${sessionFile}`,
    );
  return resolved;
}
...
export function resolveAsyncResumeTarget(
  ...
  validateStatusForResume(
    status,
    location.asyncDir
      ? path.join(location.asyncDir, "status.json")
      : "status.json",
  );
  const result = location.resultPath
    ? readResultFile(location.resultPath)
    : undefined;
  const runId =
    status?.runId ??
    result?.runId ??
  ...
  const terminalStepStatuses = new Set([
    "complete",
    "completed",
    "failed",
    "paused",
  ...
  if (stepCount > 1 && requestedIndex === undefined) {
    throw new Error(
      `Async run '${runId}' has ${stepCount} children. Provide index to choose one.`,
    );
  }
  ...
  const agent =
    statusSteps[index]?.agent ?? resultSteps[index]?.agent ?? result?.agent;
  if (!agent)
    throw new Error(
      `Could not determine child agent for async run '${runId}'.`,
    );
  const sessionFile =
...
export function buildRevivedAsyncTask(
  ...
  return [
    ...
    target.sessionFile
      ? `Original session file: ${target.sessionFile}`
      : undefined,
    "",
