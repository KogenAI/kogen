...
} from "./parallel-groups.ts";
import {
  reconcileAsyncRun,
  reconcileNestedAsyncDescendants,
} from "./stale-run-reconciler.ts";
...
function isAsyncRunDir(root: string, entry: string): boolean {
  ...
  try {
    ...
    throw new Error(
      `Failed to inspect async run path '${entryPath}': ${getErrorMessage(error)}`,
      {
...
function deriveAsyncActivityState(
  ...
  const currentStep =
    typeof status.currentStep === "number"
      ? status.steps?.[status.currentStep]
      : undefined;
  return {
    activityState: status.activityState,
    lastActivityAt:
...
function statusToSummary(
  ...
  if (status.sessionId !== undefined && typeof status.sessionId !== "string") {
    throw new Error(
      `Invalid async status '${path.join(asyncDir, "status.json")}': sessionId must be a string.`,
    );
  }
  const { activityState, lastActivityAt } = deriveAsyncActivityState(
    asyncDir,
    status,
  );
  ...
  if (nestedWarnings.length === 0) {
    try {
      nestedChildren =
        projectNestedRegistryForRoot(status.runId || path.basename(asyncDir))
          ?.children ?? [];
    } catch (error) {
      nestedWarnings.push(
        `Nested status unavailable: ${getErrorMessage(error)}`,
      );
    }
  }
...
export function listAsyncRuns(
  ...
  for (const entry of entries) {
    ...
    try {
      const nestedRoute = findNestedRouteForRootId(
        status.runId || path.basename(asyncDir),
      );
      if (nestedRoute)
...
function formatActivityFacts(input: {
  ...
  return activity || facts.length
    ? [activity, ...facts].filter(Boolean).join(" | ")
    : undefined;
}
...
export function formatAsyncRunOutputPath(
  ...
  return path.isAbsolute(run.outputFile)
    ? run.outputFile
    : path.join(run.asyncDir, run.outputFile);
}
...
export function formatAsyncRunProgressLabel(
  ...
  const activeGroup =
    run.currentStep !== undefined
      ? groups.find(
          (group) =>
            run.currentStep! >= group.start &&
            run.currentStep! < group.start + group.count,
        )
      : undefined;
  if (activeGroup) {
    const groupSteps = run.steps.slice(
      activeGroup.start,
      activeGroup.start + activeGroup.count,
    );
  ...
  if (run.mode === "parallel")
    return formatParallelOutcome(run.steps, stepCount, {
      showRunning: run.state === "running",
    });
  if (
    run.mode === "chain" &&
    ...
    const logicalStep = flatToLogicalStepIndex(
      run.currentStep,
      chainStepCount,
      groups,
    );
...
export function formatAsyncRunList(
  ...
  for (const run of runs) {
    ...
    const attached = new Set(
      run.steps.flatMap(
        (step) => step.children?.map((child) => child.id) ?? [],
      ),
    );
    const unattached =
      run.nestedChildren?.filter((child) => !attached.has(child.id)) ?? [];
    lines.push(
      ...formatNestedRunStatusLines(unattached, { indent: "  ", maxLines: 12 }),
    );
