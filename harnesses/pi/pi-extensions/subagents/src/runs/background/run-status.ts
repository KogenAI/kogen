...
function formatResumeGuidance(
  ...
  const singleSessionFile =
    knownChildren[0]?.child.sessionFile ?? fallbackSessionFile;
  if (
    children.length === 1 &&
...
function formatNestedExactStatus(
  ...
  lines.push(
    ...formatNestedRunStatusLines(run.children, {
      indent: "  ",
      commandHints: true,
    }),
  );
...
export function inspectSubagentStatus(
  ...
  if (!asyncDir && !resultPath) {
    return {
      content: [
        { type: "text", text: "Async run not found. Provide id or dir." },
      ],
      isError: true,
      details: { mode: "single", results: [] },
    };
  ...
  if (asyncDir) {
    ...
    if (status) {
      ...
      try {
        ...
        nestedChildren =
          projectNestedRegistryForRoot(status.runId)?.children ?? [];
        attachRootChildrenToSteps(status.runId, status.steps, nestedChildren);
      } catch (error) {
        nestedWarning = `Nested status unavailable: ${error instanceof Error ? error.message : String(error)}`;
      }
      const outputPath = formatAsyncRunOutputPath({
        asyncDir,
      ...
      const updated = status.lastUpdate
        ? new Date(status.lastUpdate).toISOString()
        : "n/a";
      ...
      const lines = [
        ...
        reconciliation.message
          ? `Diagnosis: ${reconciliation.message}`
          : undefined,
      ...
      for (const [index, step] of (status.steps ?? []).entries()) {
        ...
        lines.push(
          `${stepLineLabel(status, index)}: ${step.agent} ${step.status}${modelText}${stepActivityText ? `, ${stepActivityText}` : ""}${errorText}`,
        );
        lines.push(
          ...formatNestedRunStatusLines(step.children, {
            indent: "  ",
      ...
      const attached = new Set(
        (status.steps ?? []).flatMap(
          (step) => step.children?.map((child) => child.id) ?? [],
        ),
      );
      const unattached = nestedChildren.filter(
        (child) => !attached.has(child.id),
      );
      lines.push(
    ...
    try {
      ...
      return {
        content: [
          {
            type: "text",
            text: `Failed to read async result file: ${message}`,
          },
        ],
        isError: true,
        details: { mode: "single", results: [] },
