...
const CONTROL_EVENT_TYPES: ControlEventType[] = [
  "active_long_running",
  "needs_attention",
];
const CONTROL_NOTIFICATION_CHANNELS: ControlNotificationChannel[] = [
  "event",
  "async",
  "intercom",
];
const DEFAULT_NOTIFY_ON: ControlEventType[] = [
  "active_long_running",
  "needs_attention",
];
...
function parseControlList<T extends string>(
  ...
  const parsed = value.filter(
    (entry): entry is T =>
      typeof entry === "string" && allowedSet.has(entry as T),
  );
...
export function resolveControlConfig(
  ...
  const enabled =
    ...
    DEFAULT_CONTROL_CONFIG.enabled;
...
export function deriveActivityState(input: {
  ...
  return ageMs > input.config.needsAttentionAfterMs
    ? "needs_attention"
    : undefined;
}
...
export function buildControlEvent(input: {
  ...
  const type =
    input.type ??
    (input.to === "active_long_running"
      ? "active_long_running"
      : "needs_attention");
  const elapsedMs =
    input.elapsedMs ??
    (input.lastActivityAt ? Math.max(0, ts - input.lastActivityAt) : undefined);
  const elapsedSeconds =
    elapsedMs !== undefined ? Math.floor(elapsedMs / 1000) : undefined;
  const message =
    input.message ??
  ...
  return {
    ...
    reason:
      input.reason ??
      (type === "active_long_running" ? "active_long_running" : "idle"),
...
export function controlNotificationKey(
  event: ControlEvent,
