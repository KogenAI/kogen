...
const GENERIC_GLOBAL_CONFIG_PATH = path.join(
  os.homedir(),
  ".config",
  "mcp",
  "mcp.json",
...
function expandImports(config: McpConfig, cwd: string): McpConfig {
  ...
  for (const importKind of config.imports) {
    ...
    for (const [name, definition] of Object.entries(
      extractServers(imported, importKind),
    )) {
...
function resolveDirectToolNames(
  ...
  const { servers: selectedServers, tools: selectedTools } =
...
function isToolExcluded(
  ...
  return excludeTools.some(
    (excluded) =>
      ...
      candidates.has(normalizeToolName(excluded)),
  );
}
