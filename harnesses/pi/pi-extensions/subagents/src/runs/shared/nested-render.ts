...
function formatNestedRunLines(
  ...
  const append = (
    ...
    for (let index = 0; index < items.length; index++) {
      ...
      const activity =
        child.state === "running" ? formatNestedActivity(child) : undefined;
      ...
      if (options.commandHints && lines.length < options.maxLines)
        lines.push(
          `${indent}  Status: subagent({ action: "status", id: "${child.id}" })`,
        );
      if (depth === options.maxDepth) {
      ...
      for (const [stepIndex, step] of (child.steps ?? []).entries()) {
        ...
        const stepActivity =
          step.status === "running" ? formatNestedActivity(step) : undefined;
        lines.push(
          `${indent}  ${stepIndex + 1}. ${step.agent} ${step.status}${stepActivity ? ` | ${stepActivity}` : ""}${step.error ? ` | error: ${step.error}` : ""}`,
        );
