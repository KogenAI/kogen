...
export function resolveExpectedWorktreeAgentCwd(
  cwd: string,
  runId: string,
  index: number,
): string {
...
function linkNodeModulesIfPresent(
  toplevel: string,
  worktreePath: string,
): boolean {
...
function parseHookTimeout(timeoutMs: number | undefined): number {
  ...
  if (!Number.isInteger(timeoutMs) || timeoutMs <= 0) {
    throw new Error(
      "worktree setup hook timeout must be an integer greater than 0",
    );
  }
  return timeoutMs;
}
...
function resolveWorktreeSetupHook(
  ...
  if (path.isAbsolute(expandedHookPath)) {
    ...
    expandedHookPath.includes("/") ||
    expandedHookPath.includes("\\")
  ) {
    resolvedPath = path.resolve(repoRoot, expandedHookPath);
  } else {
    throw new Error(
      "worktree setup hook must be an absolute path or a repo-relative path",
    );
  }
  ...
  if (fs.statSync(resolvedPath).isDirectory()) {
    throw new Error(
      `worktree setup hook must be a file, got directory: ${resolvedPath}`,
    );
  }
...
function normalizeSyntheticPath(worktreePath: string, rawPath: string): string {
  ...
  if (!relative || relative === ".") {
    throw new Error(
      `synthetic path cannot target the worktree root: ${rawPath}`,
    );
  }
  if (
    relative === ".." ||
...
function parseWorktreeSetupHookOutput(
  rawStdout: string,
): WorktreeSetupHookOutput {
  const trimmed = rawStdout.trim();
  if (!trimmed) {
    throw new Error(
      "worktree setup hook returned empty stdout; expected JSON object",
    );
  }
  let parsed: unknown;
  try {
...
function runWorktreeSetupHook(
  ...
  if (result.error) {
    ...
    if (code === "ETIMEDOUT") {
      throw new Error(
        `worktree setup hook timed out after ${hook.timeoutMs}ms`,
      );
    }
  ...
  if (result.status !== 0) {
    ...
    throw new Error(
      `worktree setup hook failed with exit code ${result.status}: ${details}`,
    );
  }
  ...
  for (const candidate of output.syntheticPaths) {
    ...
    const normalizedPath = normalizeSyntheticPath(
      input.worktreePath,
      candidate,
    );
...
function createSingleWorktree(
  ...
  const agentCwd = cwdRelative
    ? path.join(worktreePath, cwdRelative)
    : worktreePath;
  try {
...
function captureWorktreeDiff(
  ...
  const numstat = runGitChecked(worktree.path, [
    "diff",
    "--cached",
    "--numstat",
    setup.baseCommit,
