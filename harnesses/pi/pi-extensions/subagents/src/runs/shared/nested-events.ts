...
export const NESTED_EVENTS_DIR = path.join(
  TEMP_ROOT_DIR,
...
function commonRouteRoot(
  route: Pick<NestedRoute, "eventSink" | "controlInbox">,
): string {
...
function validateRouteShape(route: NestedRoute): void {
  ...
  if (!containedPath(NESTED_EVENTS_DIR, route.eventSink))
    throw new Error(
      "Nested event sink is outside the subagent nested event root.",
    );
  if (!containedPath(NESTED_EVENTS_DIR, route.controlInbox))
    throw new Error(
      "Nested control inbox is outside the subagent nested event root.",
    );
  if (commonRouteRoot(route) !== path.dirname(path.resolve(route.controlInbox)))
    throw new Error(
      "Nested event sink and control inbox must share one route root.",
    );
}
...
export function resolveNestedRouteFromEnv(
  ...
  if (
    metadata.rootRunId !== rootRunId ||
    metadata.capabilityToken !== capabilityToken
  ) {
    throw new Error(
...
export function resolveNestedParentAddressFromEnv(
  ...
  const parentStepIndex =
    rawIndex && /^\d+$/.test(rawIndex) ? Number(rawIndex) : undefined;
...
function sanitizeTokenUsage(
  value: unknown,
): NestedRunSummary["totalTokens"] | undefined {
...
function sanitizeStep(
  ...
  return {
    agent,
    status,
    ...(stringValue(raw.sessionFile, 2048)
      ? { sessionFile: stringValue(raw.sessionFile, 2048) }
      : {}),
    ...(raw.activityState === "active_long_running" ||
    raw.activityState === "needs_attention"
    ...
    ...(clampNumber(raw.currentToolStartedAt) !== undefined
      ...
      : {}),
    ...(stringValue(raw.currentPath, 2048)
      ? { currentPath: stringValue(raw.currentPath, 2048) }
      : {}),
    ...(clampNumber(raw.turnCount) !== undefined
      ? { turnCount: clampNumber(raw.turnCount) }
      : {}),
    ...(clampNumber(raw.toolCount) !== undefined
      ? { toolCount: clampNumber(raw.toolCount) }
      : {}),
    ...(clampNumber(raw.startedAt) !== undefined
      ? { startedAt: clampNumber(raw.startedAt) }
      : {}),
    ...(clampNumber(raw.endedAt) !== undefined
      ? { endedAt: clampNumber(raw.endedAt) }
      : {}),
    ...(stringValue(raw.error, 1024)
      ? { error: stringValue(raw.error, 1024) }
      : {}),
...
export function sanitizeSummary(
  ...
  return {
    ...
    ...(stringValue(raw.asyncDir, 2048)
      ? { asyncDir: stringValue(raw.asyncDir, 2048) }
      : {}),
    ...
    Number.isInteger(clampNumber(raw.pid))
      ...
      : {}),
    ...(stringValue(raw.sessionId, 256)
      ? { sessionId: stringValue(raw.sessionId, 256) }
      : {}),
    ...(stringValue(raw.sessionFile, 2048)
      ? { sessionFile: stringValue(raw.sessionFile, 2048) }
      : {}),
    ...
    ...(raw.ownerState === "live" ||
    raw.ownerState === "gone" ||
    raw.ownerState === "unknown"
      ? { ownerState: raw.ownerState }
      : {}),
    ...(stringValue(raw.controlInbox, 2048)
      ? { controlInbox: stringValue(raw.controlInbox, 2048) }
      : {}),
    ...
    ...(raw.mode === "single" || raw.mode === "parallel" || raw.mode === "chain"
      ? { mode: raw.mode }
      : {}),
    ...(stringValue(raw.agent, 128)
      ? { agent: stringValue(raw.agent, 128) }
      : {}),
...
export function parseNestedEventRecords(
  content: string,
  route: NestedRoute,
): NestedEventRecord[] {
  if (!content.includes("\n")) {
...
function attachChild(
  ...
  const nextChild = mergeSummary(
    childIndex >= 0 ? next[childIndex] : undefined,
    event,
  );
  return childIndex >= 0
...
export function findNestedRouteForRootId(
  rootRunId: string,
): NestedRoute | undefined {
  assertSafeId("rootRunId", rootRunId);
  let entries: string[];
  ...
  for (const entry of entries) {
    ...
    try {
      const metadata = JSON.parse(
        fs.readFileSync(path.join(routeRoot, ROUTE_FILE), "utf-8"),
      ) as { rootRunId?: unknown; capabilityToken?: unknown };
      if (
        metadata.rootRunId !== rootRunId ||
...
export function projectNestedRegistryForRoot(
  rootRunId: string,
): NestedRegistry | undefined {
...
function collectScopedNestedRuns(
  ...
  for (const child of children ?? []) {
    if (
      ...
      (scope.parentStepIndex === undefined ||
        child.parentStepIndex === scope.parentStepIndex)
    ) {
...
export function findNestedRunMatchesById(
  ...
  for (const route of options.scope?.routes ?? listNestedRoutes()) {
    try {
      ...
      for (const run of collectScopedNestedRuns(
        registry.children,
        options.scope?.descendantOf,
      )) {
...
export function readNestedRegistry(route: NestedRoute): NestedRegistry {
  ...
  try {
    const parsed = JSON.parse(
      fs.readFileSync(registryPath(route), "utf-8"),
    ) as NestedRegistry;
    return {
      rootRunId: route.rootRunId,
...
export function writeNestedControlRequest(
  ...
  request: Omit<
    ...
    "type" | "rootRunId" | "capabilityToken"
  >,
): string {
  validateRouteShape(route);
...
export function readNestedControlResults(
  route: NestedRoute,
): NestedControlResultRecord[] {
  validateRouteShape(route);
  let entries: string[] = [];
...
>(
  ...
  for (const child of children) {
    ...
    const step = steps.find(
      (candidate, index) =>
        (candidate.index ?? index) === child.parentStepIndex,
    );
    if (!step) continue;
    step.children ??= [];
    step.children = [
...
export function nestedSummaryFromAsyncStatus(
  ...
  return {
    ...
    path: fallback.path ?? [
      {
        ...
        ...(fallback.parentStepIndex !== undefined
          ? { stepIndex: fallback.parentStepIndex }
          : {}),
      },
    ],
    asyncDir,
    ...
    ...(status.currentStep !== undefined
      ...
      : {}),
    ...(status.chainStepCount !== undefined
...
export function nestedArtifactEnv(
  rootRunId: string,
  parentRunId: string,
