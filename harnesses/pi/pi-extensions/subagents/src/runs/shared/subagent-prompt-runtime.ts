...
const SUBAGENT_INHERIT_PROJECT_CONTEXT_ENV =
...
const SUBAGENT_ORCHESTRATION_SKILL_NAME_PATTERN =
  /<name>\s*pi-subagents\s*<\/name>/;
...
export function stripProjectContext(prompt: string): string {
  ...
  const endIndex = findSectionEnd(
    prompt,
    startIndex + PROJECT_CONTEXT_HEADER.length,
...
export function stripInheritedSkills(prompt: string): string {
  ...
  if (startIndex === -1) return prompt;
  const endIndex = findSectionEnd(prompt, startIndex + SKILLS_HEADER.length, [
    DATE_HEADER,
  ]);
  return `${prompt.slice(0, startIndex)}${prompt.slice(endIndex)}`;
}
...
export function rewriteSubagentPrompt(
  ...
  const boundary = options.fanoutChild
    ? CHILD_FANOUT_BOUNDARY_INSTRUCTIONS
    : CHILD_SUBAGENT_BOUNDARY_INSTRUCTIONS;
  return `${boundary}\n\n${rewritten}`;
}
...
export function stripParentOnlySubagentMessages(
  ...
  for (const message of messages) {
    if (
      ...
      isSubagentToolResultMessage(message)
    ) {
      changed = true;
      continue;
    }
...
export default function registerSubagentPromptRuntime(pi: ExtensionAPI): void {
  ...
  pi.on("before_agent_start", async (event) => {
    ...
    const inheritProjectContext = readBooleanEnv(
