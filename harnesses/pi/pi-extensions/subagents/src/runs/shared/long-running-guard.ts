...
export function nextLongRunningTrigger(
  ...
  if (
    config.activeNoticeAfterTurns !== undefined &&
    metrics.turns >= config.activeNoticeAfterTurns
  )
...
export function recordMutatingFailure(
  ...
  if (
    state.lastFailureAt === undefined ||
    input.ts - state.lastFailureAt > windowMs
  ) {
    state.consecutiveFailures = 0;
    state.recentFailures = [];
    state.repeatedPathFailures = 0;
...
export function shouldEscalateMutatingFailures(
  ...
  return (
    ...
    state.repeatedPathFailures >= threshold
  );
}

export function summarizeRecentMutatingFailures(
  state: MutatingFailureState,
): string | undefined {
