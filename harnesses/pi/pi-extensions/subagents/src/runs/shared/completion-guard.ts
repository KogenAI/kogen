...
function stripFrameworkInstructions(task: string): string {
  return task
    ...
    .filter(
      (line) =>
        !/^\s*(?:Create and maintain progress at:|Update progress at:|Write your findings to:)/i.test(
          line,
        ),
    )
    .join("\n");
...
export function expectsImplementationMutation(
  ...
  const taskTextWithoutScopedConstraints =
    stripScopedNoEditConstraints(taskText);
  if (
    REVIEW_ONLY_PATTERNS.some((pattern) =>
      pattern.test(taskTextWithoutScopedConstraints),
    )
  )
    return false;
  if (
    EXPLICIT_NO_EDIT_PATTERNS.some((pattern) =>
      pattern.test(taskTextWithoutScopedConstraints),
    )
  )
    return false;
  ...
  if (/\breviewer\b/i.test(agent))
    return REVIEWER_REQUIRED_EDIT_PATTERNS.some((pattern) =>
  ...
  return GENERAL_IMPLEMENTATION_PATTERNS.some((pattern) =>
    pattern.test(taskText),
  );
}
...
export function evaluateCompletionMutationGuard(
  ...
  const expectedMutation = declaresOnlyReadOnlyTools(
    input.tools,
