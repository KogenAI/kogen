...
function compactNestedRun(
  ...
  return {
    ...
    ...(run.ownerIntercomTarget
      ? { ownerIntercomTarget: run.ownerIntercomTarget }
      : {}),
    ...(run.leafIntercomTarget
      ? { leafIntercomTarget: run.leafIntercomTarget }
      : {}),
    ...
    ...(run.chainStepCount !== undefined
      ? { chainStepCount: run.chainStepCount }
      : {}),
    ...(run.parallelGroups?.length
      ? { parallelGroups: run.parallelGroups.slice(0, 8) }
      : {}),
...
export function attachNestedChildrenToResultChildren(
  ...
  return children.map((child, index) => {
    ...
    const alreadyAttachedIds = new Set(
      child.children?.map((nested) => nested.id) ?? [],
    );
...
function formatNestedResultLines(
  ...
  const append = (
    ...
    for (const run of runs ?? []) {
      if (remaining <= 0) {
        lines.push(
          `${indent}↳ +more nested runs; inspect status for full tree`,
        );
        return;
      }
      remaining--;
