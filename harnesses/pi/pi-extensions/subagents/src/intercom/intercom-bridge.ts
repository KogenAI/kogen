...
function resolveIntercomBridgeConfig(
  ...
  return {
    ...
    instructionFile:
      typeof value.instructionFile === "string" ? value.instructionFile : "",
  };
}

function intercomConfigStatus(configPath: string): {
  enabled: boolean;
  error?: unknown;
} {
  ...
  try {
    ...
    const code =
      error && typeof error === "object" && "code" in error
        ...
        : undefined;
    if (code !== "ENOENT")
      console.warn(`Failed to read JSON from '${filePath}'.`, error);
    return null;
  }
}
...
function configuredPiIntercomPackageDir(
  input: ResolveIntercomBridgeInput,
  agentDir: string,
): string | undefined {
  const projectConfigDir = input.cwd
    ? findNearestProjectConfigDir(path.resolve(input.cwd))
    : undefined;
  const settingsFiles = [
    ...(projectConfigDir
      ? [
...
function resolveIntercomExtensionDir(
  input: ResolveIntercomBridgeInput,
  agentDir: string,
): string {
  const legacyDir = path.resolve(
    input.extensionDir ?? defaultIntercomExtensionDir(agentDir),
  );
...
function resolveInstructionTemplate(
...
function buildIntercomBridgeInstruction(
  orchestratorTarget: string,
  template: string,
): string {
  const instruction = template
    .replaceAll("{orchestratorTarget}", orchestratorTarget)
    .trim();
...
export function diagnoseIntercomBridge(
  ...
  const configPath = path.resolve(
    input.configPath ?? defaultIntercomConfigPath(agentDir),
  );
  const wantsIntercom =
    mode !== "off" && !(mode === "fork-only" && input.context !== "fork");
...
export function resolveIntercomBridge(
  input: ResolveIntercomBridgeInput,
): IntercomBridgeState {
  const config = resolveIntercomBridgeConfig(input.config);
  const mode = config.mode;
  ...
  const settingsDir = path.resolve(
    input.settingsDir ?? defaultSubagentConfigDir(agentDir),
  );
  ...
  if (intercomStatus.error)
    console.warn(
      `Failed to parse intercom config at '${configPath}'. Assuming enabled.`,
