...
function parsePackageConfig(value: unknown): {
  packageName?: string;
  error?: string;
} {
...
function availableNames(cwd: string, kind: "agent" | "chain"): string[] {
  ...
  return [...new Set(items.map((x) => x.name))].sort((a, b) =>
    a.localeCompare(b),
  );
}
...
function findChains(
  ...
  return discoverAgentsAll(cwd)
    .chains.filter(
      (c) =>
        (scope === "both" || c.source === scope) &&
        (c.name === raw || c.name === sanitized),
    )
    .sort((a, b) => a.source.localeCompare(b.source));
}
...
function unknownChainAgents(cwd: string, steps: ChainStepConfig[]): string[] {
  ...
  return [
    ...new Set(steps.map((s) => s.agent).filter((a) => !known.has(a))),
  ].sort((a, b) => a.localeCompare(b));
}

function chainStepWarnings(
  ...
  for (let i = 0; i < steps.length; i++) {
    ...
    if (Array.isArray(s.skills) && s.skills.length > 0) {
      ...
      if (missing.length)
        warnings.push(
          `Warning: step ${i + 1} (${s.agent}): skills not found: ${missing.join(", ")}.`,
        );
    }
  }
  return warnings;
...
function modelWarning(
  ...
  const found = ctx.modelRegistry
    .getAvailable()
    .some((m) => `${m.provider}/${m.id}` === model || m.id === model);
  return found
    ? undefined
    : `Warning: model '${model}' is not in the current model registry.`;
}
...
function fallbackModelsWarning(
  ...
  const available = new Set(
    ctx.modelRegistry
      .getAvailable()
      .flatMap((m) => [`${m.provider}/${m.id}`, m.id]),
  );
...
function skillsWarning(
  ...
  return missing.length
    ? `Warning: skills not found: ${missing.join(", ")}.`
    : undefined;
}

function parseStepList(raw: unknown): {
  steps?: ChainStepConfig[];
  error?: string;
} {
  ...
  for (let i = 0; i < raw.length; i++) {
    ...
    if (hasKey(s, "outputMode")) {
      if (s.outputMode === "inline" || s.outputMode === "file-only")
        ...
        return {
          error: `config.steps[${i}].outputMode must be 'inline' or 'file-only'.`,
        };
    }
    if (hasKey(s, "reads")) {
...
function resolveTarget<T extends { source: AgentSource; filePath: string }>(
  ...
  if (scoped.length === 0)
    return result(
      `${kind === "agent" ? "Agent" : "Chain"} '${name}' not found in scope '${scope}'.`,
      true,
    );
  if (scoped.length > 1)
    return result(
...
function formatChainDetail(chain: ChainConfig): string {
  ...
  for (let i = 0; i < chain.steps.length; i++) {
    ...
    if (s.progress !== undefined)
      lines.push(`   Progress: ${s.progress ? "true" : "false"}`);
  }
  return lines.join("\n");
}
...
export function handleList(
  ...
  const scopedAgents = allAgents(d)
    .filter(
      (a) => scope === "both" || a.source === "builtin" || a.source === scope,
    )
  ...
  const lines = [
    ...
    "Chains:",
    ...(chains.length
      ? chains.map((c) => `- ${c.name} (${c.source}): ${c.description}`)
      : ["- (none)"]),
  ];
...
export function handleCreate(
  ...
  const targetDir = isChain
    ? scope === "user"
      ? d.userChainDir
      : (d.projectChainDir ?? path.join(ctx.cwd, ".pi", "chains"))
    : scope === "user"
      ? d.userDir
      : (d.projectDir ?? path.join(ctx.cwd, ".pi", "agents"));
  fs.mkdirSync(targetDir, { recursive: true });
  if (nameExistsInScope(ctx.cwd, scope, runtimeName))
    return result(
      `Name '${runtimeName}' already exists in ${scope} scope. Use update instead.`,
      true,
    );
  const targetPath = path.join(
    targetDir,
    isChain ? `${runtimeName}.chain.md` : `${runtimeName}.md`,
  );
  ...
  if (isChain) {
    ...
    return result(
      [`Created chain '${runtimeName}' at ${targetPath}.`, ...warnings].join(
        "\n",
      ),
    );
  }
  ...
  return result(
    [`Created agent '${runtimeName}' at ${targetPath}.`, ...warnings].join(
      "\n",
    ),
  );
}
...
export function handleUpdate(
  ...
  if (params.agent) {
    ...
    if (updated.name !== oldName) {
      ...
      if (refs.length)
        warnings.push(
          `Warning: chains still reference '${oldName}': ${refs.join(", ")}.`,
        );
    }
    const headline =
