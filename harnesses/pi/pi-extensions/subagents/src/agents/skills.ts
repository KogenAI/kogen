...
function collectInstalledPackageSkillPaths(
  cwd: string,
  agentDir: string,
): SkillSearchPath[] {
  const dirs: SkillSearchPath[] = [
    ...
    {
      path: path.join(agentDir, "npm", "node_modules"),
      source: "user-package",
    },
  ];
    ...
    for (const entry of entries) {
      ...
      if (entry.name.startsWith("@")) {
        ...
        for (const scopeEntry of scopeEntries) {
          ...
          results.push(
            ...extractSkillPathsFromPackageRoot(pkgRoot, dir.source, true),
          );
        }
        continue;
      }
...
function collectSettingsSkillPaths(
  cwd: string,
  agentDir: string,
): SkillSearchPath[] {
  const results: SkillSearchPath[] = [];
...
function collectSettingsPackageSkillPaths(
  cwd: string,
  agentDir: string,
): SkillSearchPath[] {
  const settingsFiles = [
...
function inferSkillSource(
  ...
  if (
    ...
    isWithinPath(filePath, projectAgentsRoot)
  )
    return "project";
  ...
  if (
    ...
    isWithinPath(filePath, userAgentsRoot)
  )
    return "user";
