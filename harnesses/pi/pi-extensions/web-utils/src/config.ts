...
const BUILTIN_ENGINES: Record<Exclude<SearchEngineKind, "custom">, Omit<ResolvedSearchEngineConfig, "id">> = {
	searxng: {
		kind: "searxng",
...
function mergeEngine(base: SearchEngineConfig | undefined, update: SearchEngineConfig): SearchEngineConfig {
	return {
		...base,
		...update,
		queryParams: { ...(base?.queryParams ?? {}), ...(update.queryParams ?? {}) },
		headers: { ...(base?.headers ?? {}), ...(update.headers ?? {}) },
	};
}

function sanitizeEngine(engine: SearchEngineConfig): ResolvedSearchEngineConfig | null {
	...
	return {
		...
		queryParams: { ...(builtin?.queryParams ?? {}), ...(engine.queryParams ?? {}) },
...
function resolveSearchConfig(raw: WebToolsConfig): ResolvedSearchConfig {
	...
	let fallbackOrder = (user.fallbackOrder ?? []).filter((id) => available.has(id));
	if (fallbackOrder.length === 0) fallbackOrder = engines.map((engine) => engine.id);
...
function resolveFetchConfig(raw: WebToolsConfig): ResolvedFetchConfig {
	...
	const markdownNewBase = ensureTrailingSlash(user.markdownNew?.baseUrl ?? "https://markdown.new/");
...
function resolveLocalSearchConfig(raw: WebToolsConfig): ResolvedLocalSearchConfig {
	const user = raw.localSearch ?? {};
	const maxMatches = toPositiveInt(user.maxMatches, 300);
	const defaultMaxMatches = Math.min(toPositiveInt(user.defaultMaxMatches, 80), maxMatches);
	return {
		defaultMaxMatches,
		maxMatches,
