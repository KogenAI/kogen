...
import { fetchText, isProbablyHtml, isProbablyJson, truncateText } from "../utils/http.js";
...
const FetchPageParamsSchema = Type.Object({
	...
	preferMarkdownNew: Type.Optional(
		Type.Boolean({ description: "Try markdown.new first (https://markdown.new/<url>) before local HTML processing" }),
	),
	maxChars: Type.Optional(Type.Integer({ minimum: 1000, maximum: 500000, description: "Max output size" })),
	includeRawHtml: Type.Optional(Type.Boolean({ description: "Include truncated raw HTML in details" })),
});
...
async function tryMarkdownNew(
	...
	try {
		...
		if (body.startsWith("{") || body.toLowerCase().startsWith("error:")) return null;
...
async function fetchAndConvert(
	...
	if (output === "json") {
		...
		return {
			text: truncateText(
				JSON.stringify({ url: response.url, contentType: response.contentType, content: response.body }, null, 2),
				maxChars,
			),
			details,
		};
	...
	if (isProbablyHtml(response.contentType, response.body)) {
		...
		return {
			...
			details: {
				...
				rawHtml: params.includeRawHtml ? truncateText(response.body, 12000) : undefined,
			},
		};
	}
...
export function createFetchWebpageTool(): ToolDefinition<typeof FetchPageParamsSchema> {
	return {
		name: "fetch_webpage",
		label: "Fetch Webpage",
		description:
		...
		async execute(_toolCallId, params, signal, onUpdate) {
			...
			onUpdate?.({
				content: [{ type: "text", text: "markdown.new unavailable, using local HTML conversion..." }],
			...
			try {
				...
				return {
					content: [
						{ type: "text", text: `Failed to fetch webpage: ${error instanceof Error ? error.message : String(error)}` },
					],
