...
const SearchParamsSchema = Type.Object({
  ...
  engineId: Type.Optional(
    Type.String({
      description: "Preferred engine id from config.search.engines",
    }),
  ),
  fallbackOrder: Type.Optional(
    Type.Array(Type.String(), {
      description: "Override fallback order for this call",
    }),
  ),
  ...
  allowFallback: Type.Optional(
    Type.Boolean({
      description: "Try next configured engines if preferred engine fails",
    }),
  ),
  rawHtmlFormat: Type.Optional(
    Type.Union(
      [Type.Literal("none"), Type.Literal("markdown"), Type.Literal("json")],
      {
        description:
          "If engine returns HTML, optionally format raw HTML using webpage fetch formatter",
      },
    ),
  ),
  includeRawResponse: Type.Optional(
    Type.Boolean({
      description: "Include raw engine response (truncated) in details",
    }),
  ),
});
...
function parseDuckDuckGoHtml(
  ...
  for (const node of Array.from(document.querySelectorAll(".result"))) {
    ...
    const snippet =
      node.querySelector(".result__snippet")?.textContent?.trim() ?? undefined;
...
function parseGoogleHtml(
  ...
  for (const block of Array.from(document.querySelectorAll("div.g"))) {
    ...
    const title =
      ...
      url;
    const snippet =
      block.querySelector("div.VwiC3b")?.textContent?.trim() ?? undefined;
    hits.push({ title, url, snippet });
    if (hits.length >= maxResults) break;
  }
...
function parseHtmlHits(
  ...
  if (kind === "duckduckgo")
    return parseDuckDuckGoHtml(html, baseUrl, maxResults);
...
async function searchWithEngine(
  ...
  if (engine.responseFormat === "auto") {
    mode = isProbablyJson(response.contentType, response.body)
      ? "json"
      : "html";
  } else {
  ...
  if (mode === "json") {
    ...
    if (
      hits.length === 0 &&
      isProbablyHtml(response.contentType, response.body)
    ) {
      hits = parseHtmlHits(
        engine.kind,
        response.body,
  ...
  if (
    mode === "html" &&
    params.rawHtmlFormat &&
    params.rawHtmlFormat !== "none"
  ) {
    if (params.rawHtmlFormat === "markdown") {
      formattedHtml = htmlToMarkdown(response.body, response.url).markdown;
    } else {
      formattedHtml = JSON.stringify(
        htmlToStructuredJson(response.body, response.url),
        null,
        2,
      );
    }
  ...
  return {
    ...
    formattedHtml,
    rawResponse: params.includeRawResponse
  ...
  return {
    ...
    async execute(_toolCallId, params, signal, onUpdate, ctx) {
      ...
      const enginesById = new Map(
        config.search.engines.map((engine) => [engine.id, engine]),
      );
      ...
      if (enginesById.size === 0) {
        return {
          content: [
            {
              type: "text",
              text: "No search engines configured. Add search.engines to ~/.pi/web-tools.json.",
            },
          ],
      ...
      const requestedOrder =
        params.fallbackOrder?.filter((id) => enginesById.has(id)) ?? [];
      const fallbackOrder =
        requestedOrder.length > 0
          ? requestedOrder
          : config.search.fallbackOrder;
        ...
        if (!enginesById.has(params.engineId)) {
          return {
            content: [
              {
                type: "text",
                text: `Unknown engineId \"${params.engineId}\".`,
              },
            ],
            details: {
              error: "Unknown engineId",
              availableEngines: [...enginesById.keys()],
            },
          };
        }
      ...
      if (candidateIds.length === 0) {
        return {
          content: [
            {
              type: "text",
              text: "No candidate engines available after filtering.",
            },
          ],
      ...
      for (let index = 0; index < candidateIds.length; index += 1) {
        ...
        onUpdate?.({
          content: [
            {
              type: "text",
              text: `Searching with ${engine.id} (${index + 1}/${candidateIds.length})...`,
            },
          ],
          details: {
            phase: "search",
            engineId: engine.id,
            progress: index / candidateIds.length,
          },
        });
        try {
          const result = await searchWithEngine(
            params.query,
            maxResults,
            engine,
            params,
      ...
      const attemptSummary = attempts
        .map(
          (attempt) =>
            `- ${attempt.engineId}: ${attempt.ok ? `ok (${attempt.resultCount ?? 0} results)` : attempt.error}`,
        )
        .join("\n");
