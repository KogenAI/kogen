...
const CloneRepoParamsSchema = Type.Object({
	...
	forceClone: Type.Optional(Type.Boolean({ description: "Clone even when repo exceeds maxRepoSizeMB" })),
	refresh: Type.Optional(Type.Boolean({ description: "Delete existing cached clone and reclone" })),
	maxTreeEntries: Type.Optional(Type.Integer({ minimum: 20, maximum: 2000, description: "Tree/listing limit" })),
});
...
const LocalSearchParamsSchema = Type.Object({
	...
	repo: Type.Optional(
		Type.String({
			description: "Repo key from clone_github_repo, e.g. owner/repo or owner/repo@branch",
		}),
	),
	path: Type.Optional(
		Type.String({
			description: "Directory path to search. Relative paths resolve from selected repo root (or current cwd).",
		}),
	),
	glob: Type.Optional(Type.String({ description: "Optional include glob, passed to rg -g" })),
	maxMatches: Type.Optional(Type.Integer({ minimum: 1, maximum: 1000, description: "Maximum matches to return" })),
...
function clonePathFor(owner: string, repo: string, ref: string | undefined): string {
...
function readReadme(localPath: string): string | null {
	const names = ["README.md", "README", "README.txt", "readme.md", "README.rst"];
...
function buildTree(rootPath: string, maxEntries: number): string {
	...
	walk(rootPath, "");
	if (entries.length >= maxEntries) entries.push(`... (truncated at ${maxEntries} entries)`);
	return entries.join("\n");
}

function buildDirListing(rootPath: string, dirPath: string, maxEntries: number): string {
	...
	if (names.length > maxEntries) lines.push(`... (${names.length - maxEntries} more entries)`);
	return lines.map((line) => `  ${line}`).join("\n");
}

function renderClonePreview(localPath: string, info: GitHubUrlInfo, maxTreeEntries: number): string {
	const config = loadConfig();
	const lines: string[] = [];
	...
	if (isBinaryFile(filePath)) {
		lines.push(`## ${subPath}`);
		lines.push(`Binary file (${formatSize(stats.size)}). Inspect with read or bash if needed.`);
		return lines.join("\n");
	}
...
async function githubApi(path: string, signal?: AbortSignal): Promise<unknown | null> {
...
async function fetchRepoSizeMB(owner: string, repo: string, signal?: AbortSignal): Promise<number | null> {
...
async function fetchDefaultBranch(owner: string, repo: string, signal?: AbortSignal): Promise<string | null> {
	const data = await githubApi(`/repos/${owner}/${repo}`, signal);
	if (!data || typeof data !== "object") return null;
...
async function fetchViaApiPreview(
	...
	const config = loadConfig();
	const ref = info.ref || (await fetchDefaultBranch(info.owner, info.repo, signal));
	if (!ref) return null;
	...
	if (info.type === "blob" && info.path) {
		const file = await githubApi(`/repos/${info.owner}/${info.repo}/contents/${info.path}?ref=${encodeURIComponent(ref)}`, signal);
	...
	const readme = await githubApi(`/repos/${info.owner}/${info.repo}/readme?ref=${encodeURIComponent(ref)}`, signal);
	if (readme && typeof readme === "object" && typeof (readme as Record<string, unknown>).content === "string") {
		lines.push("");
		lines.push("## README");
		lines.push(decodeBase64((readme as Record<string, unknown>).content as string));
	}

	lines.push("");
	lines.push("API preview only. Use forceClone: true to clone the repository locally.");
...
export class GitHubToolState {
	...
	async cloneFromUrl(params: CloneRepoParams, signal: AbortSignal | undefined): Promise<{ text: string; details: Record<string, unknown> }> {
		const config = loadConfig();
		...
		const maxTreeEntries = params.maxTreeEntries ?? config.github.maxTreeEntries;
		...
		if (result.code !== 0 || !existsSync(localPath)) {
			...
			return {
				...
				details: { error: "clone failed", stderr: result.stderr, stdout: result.stdout },
			};
		}

		const record: CloneRecord = { key, owner: info.owner, repo: info.repo, ref: info.ref, localPath };
	...
	async searchLocal(
		...
		const maxMatches = Math.min(params.maxMatches ?? config.localSearch.defaultMaxMatches, config.localSearch.maxMatches);
		const caseSensitive = params.caseSensitive ?? false;
		const rgResults = await this.searchWithRg(params.query, root, maxMatches, caseSensitive, params.glob, signal);
		...
		if (rgResults.kind === "ok") {
			...
			engine = "grep";
			matches = await this.searchWithGrep(params.query, root, maxMatches, caseSensitive, signal);
		} else {
			return {
		...
		lines.push(`Matches: ${matches.length}${matches.length >= maxMatches ? ` (truncated to ${maxMatches})` : ""}`);
		lines.push("");
		...
		if (matches.length === 0) {
			...
			for (let index = 0; index < matches.length; index += 1) {
				...
				const file = match.file.startsWith(root) ? relative(root, match.file) || basename(match.file) : match.file;
	...
	private async searchWithRg(
		...
		signal: AbortSignal | undefined,
	): Promise<{ kind: "ok"; matches: SearchMatch[] } | { kind: "unavailable" } | { kind: "error"; error: string }> {
		if (!(await isRgAvailable())) return { kind: "unavailable" };

		const config = loadConfig();
		const args = ["--json", "--line-number", "--column", "--max-count", String(maxMatches)];
		...
		if (result.code !== 0 && result.code !== 1) {
			return { kind: "error", error: `rg failed: ${result.stderr || result.stdout}` };
		}
		...
		for (const line of result.stdout.split("\n")) {
			...
			try {
				...
				const submatches = Array.isArray(data.submatches) ? (data.submatches as Array<{ start?: number }>) : [];
	...
	private async searchWithGrep(
		...
		for (const line of result.stdout.split("\n")) {
			...
			const text = line.slice(second + 1).trim().slice(0, config.localSearch.previewChars);
			matches.push({ file, line: Number.isFinite(lineNumber) ? lineNumber : 0, column: 1, text });
			if (matches.length >= maxMatches) break;
		}
...
export function createCloneGitHubTool(state: GitHubToolState): ToolDefinition<typeof CloneRepoParamsSchema> {
	return {
		...
		async execute(_toolCallId, params, signal, onUpdate) {
			onUpdate?.({ content: [{ type: "text", text: "Resolving GitHub URL and cloning..." }], details: { phase: "clone" } });
...
export function createLocalRepoSearchTool(state: GitHubToolState): ToolDefinition<typeof LocalSearchParamsSchema> {
	return {
		...
		async execute(_toolCallId, params, signal, onUpdate, ctx) {
			onUpdate?.({ content: [{ type: "text", text: "Searching local repository..." }], details: { phase: "search" } });
