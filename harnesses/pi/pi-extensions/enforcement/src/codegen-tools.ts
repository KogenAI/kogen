...
function readLog(
  ...
  slug?: string,
  role?: string,
): { logPath: string; view: LogView; events: LogEvent[] } {
...
function registerSectionTool(pi: ExtensionAPI, spec: RoleSpec) {
  const SectionParams = Type.Object({
    body: Type.String({
      description: "Free-form prose body for this role's ev:role event.",
    }),
    learned: Type.Optional(
      Type.String({
        description: "Optional ev:learned text appended in the same call.",
      }),
    ),
    slug: Type.Optional(
      Type.String({ description: "Cycle slug; omit to resolve via .active." }),
    ),
  });
  ...
  pi.registerTool({
    ...
    async execute(_toolCallId, params: Static<typeof SectionParams>) {
      ...
      if (!result.ok)
        throw new Error(`codegen-log section failed: ${result.stderr}`);
...
function registerAppendTool(pi: ExtensionAPI, spec: RoleSpec) {
  ...
  const AppendParams = Type.Object({
    ...
    text: Type.Optional(
      Type.String({
        description: "Text payload for learned/no_learning/died --cause.",
      }),
    ),
    body: Type.Optional(
      Type.String({
  ...
  pi.registerTool({
    ...
    async execute(_toolCallId, params: Static<typeof AppendParams>) {
      ...
      switch (kind) {
        ...
        case "files_modified":
          if (!body)
            throw new Error(`kind=${kind} requires "body" (piped via stdin)`);
      ...
      if (!result.ok)
        throw new Error(`codegen-log append failed: ${result.stderr}`);
...
function registerReaders(pi: ExtensionAPI) {
  ...
  const LogReadParams = Type.Object({
    ...
    view: Type.Union(
      [Type.Literal("manifest"), Type.Literal("retro"), Type.Literal("full")],
      {
    ...
    role: Type.Optional(
      Type.String({ description: "Filter to one role's events." }),
    ),
  });
