...
SENTINEL4="Before each role's spawn, open its section by running \`codegen-log section --role <role> --slug <slug> --body @-\` with an EMPTY stdin body — this inserts the header ONCE, immediately before that role's spawn; never re-open a header that already exists."
