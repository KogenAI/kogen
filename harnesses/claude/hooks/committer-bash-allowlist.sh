...
if printf '%s' "$COMMAND" | grep -qE '^[[:space:]]*(git[[:space:]]+(diff|status|log|show|commit|add|rm|mv|tag|checkout|switch|branch|restore|reset)\b|echo\b|wc\b|cat\b|ls\b|true\b|:)'; then
    exit 0
fi
