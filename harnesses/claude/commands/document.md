...
4. Save automatically — kebab-case slug. `mkdir -p codegen/pitches/draft` then save to `codegen/pitches/draft/<slug>.md`. Re-running on existing slug → Edit in place; the doc is a living skeleton. Same problem under a different slug (per the dedup scan above) → Edit the existing draft, do not create a sibling.
