...
function readLog(cwd: string, view: "manifest" | "retro" | "full", slug?: string, role?: string) {
...
describe("roles.ts — role-baking", { concurrency: 1 }, () => {
    ...
    test("universal marker kinds (learned/no_learning/died) plus per-role extras", () => {
        ...
        assert.deepEqual(planner.extraKinds, ["plan", "plan_gate", "files_to_touch"]);
...
describe("exec.ts — array argv, stdin body, isError propagation", { concurrency: 1 }, () => {
    test("body is passed via stdin, never appears in argv (fake codegen-log echoes argv+stdin)", async () => {
        await withTmpDir((dir) => {
            ...
            try {
                const result = runCodegenLog(["section", "developer-phoenix-backend", "--slug", "x"], "hello body with `backticks` and $(danger)");
                assert.equal(result.ok, true);
                assert.match(result.stdout, /^ARGV:section developer-phoenix-backend --slug x$/m);
                assert.match(result.stdout, /STDIN:hello body with `backticks` and \$\(danger\)/);
...
describe("readers.ts — log_read views", { concurrency: 1 }, () => {
    ...
    test("manifest view returns only plan/plan_gate/files_to_touch events", async () => {
        await withTmpDir((dir) => {
            ...
            assert.ok(result.events.every((e) => ["plan", "files_to_touch"].includes(e.ev)));
        });
    });
    ...
    test("retro view returns only ev:learned, excludes no_learning (curator-invisible)", async () => {
        await withTmpDir((dir) => {
            writeFixtureLog(dir, [
                { ev: "learned", role: "developer-phoenix-backend", text: "caught a green-from-birth test" },
