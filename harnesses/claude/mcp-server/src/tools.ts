...
import { ROLES, RoleSpec, appendToolName, sectionToolName, MarkerKind } from "./roles";
...
function registerSectionTool(server: McpServer, spec: RoleSpec) {
    server.registerTool(
        sectionToolName(spec.role),
        {
            ...
            inputSchema: {
                body: z.string().min(1).describe("Free-form prose body for this role's ev:role event."),
                learned: z
                    .string()
                    .optional()
                    .describe("Optional ev:learned text appended in the same call."),
                slug: z.string().optional().describe("Cycle slug; omit to resolve via .active."),
            },
        },
        async ({ body, learned, slug }) => {
            ...
            if (!result.ok) return errorResult(`codegen-log section failed: ${result.stderr}`);
...
function registerAppendTool(server: McpServer, spec: RoleSpec) {
    ...
    server.registerTool(
        appendToolName(spec.role),
        {
            ...
            inputSchema: {
                kind: z.enum(allowedKinds as [MarkerKind, ...MarkerKind[]]).describe("Which marker event to append."),
                text: z
                    .string()
                    .optional()
                ...
                body: z
                    .string()
                    .optional()
                    .describe("Raw text/JSON payload for plan/plan_gate/files_to_touch/files_modified (piped via stdin)."),
                died_kind: z.enum(["interrupted", "aborted"]).optional().describe("Required when kind=died."),
                verdict_value: z
        ...
        async ({ kind, text, body, died_kind, verdict_value, slug }) => {
            if (!allowedKinds.includes(kind)) {
                return errorResult(`role "${spec.role}" is not granted marker kind "${kind}"`);
            }
            ...
            switch (kind) {
                ...
                case "died":
                    ...
                    break;
                case "verdict":
                    if (!verdict_value) return errorResult('kind=verdict requires "verdict_value"');
                ...
                case "files_to_touch":
                case "files_modified":
                    if (!body) return errorResult(`kind=${kind} requires "body" (piped via stdin)`);
            ...
            if (!result.ok) return errorResult(`codegen-log append failed: ${result.stderr}`);
...
function registerReaders(server: McpServer) {
    server.registerTool(
        "gate_status",
        {
            ...
            inputSchema: {
                cwd: z.string().describe("Absolute cwd of the active build (contains codegen/gate-pending/)."),
            },
        },
        async ({ cwd }) => {
            try {
    ...
    server.registerTool(
        "log_read",
        {
            ...
            inputSchema: {
                cwd: z.string().describe("Absolute cwd of the active build (contains codegen/logging/)."),
                view: z.enum(["manifest", "retro", "full"]).describe("Which projection to return."),
                slug: z.string().optional().describe("Cycle slug; omit to resolve via .active."),
